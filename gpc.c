...
static edge_node *
build_lmt(lmt_node ** lmt, sb_tree ** sbtree,
	...
	for (c = 0; c < p->num_contours; c++)
	{
		...
		else
		{
			...
			for (i = 0; i < p->contour[c].num_vertices; i++)
				if (OPTIMAL
					(p->contour[c].vertex, i, p->contour[c].num_vertices))
				{
			...
			for (min = 0; min < num_vertices; min++)
			{
				...
				if (FWD_MIN(edge_table, min, num_vertices))
				{
					...
					for (i = 0; i < num_edges; i++)
					{
						...
						e[i].next = NULL;
						e[i].prev = NULL;
						e[i].succ = ((num_edges > 1)
									 && (i <
										 (num_edges - 1))) ? &(e[i +
																 1]) : NULL;
						e[i].pred = ((num_edges > 1)
									 && (i > 0)) ? &(e[i - 1]) : NULL;
						e[i].next_bound = NULL;
						e[i].bside[CLIP] = (op == GPC_DIFF) ? RIGHT : LEFT;
						e[i].bside[SUBJ] = LEFT;
					}
					insert_bound(bound_list(lmt, edge_table[min].vertex.y),
								 e);
				}
			}
...
static void
add_intersection(it_node ** it, edge_node * edge0, edge_node * edge1,
				 double x, double y)
{
...
static int
count_tristrips(polygon_node * tn)
{
	int total;
...
static void
add_vertex(vertex_node ** t, double x, double y)
{
	if (!(*t))
	{
...
static void
new_tristrip(polygon_node ** tn, edge_node * edge, double x, double y)
{
	if (!(*tn))
	{
...
void
gpc_read_polygon(FILE * fp, int read_hole_flags, gpc_polygon * p)
{
	int c, v;
...
void
gpc_write_polygon(FILE * fp, int write_hole_flags, gpc_polygon * p)
{
	int c, v;
...
void
gpc_add_contour(gpc_polygon * p, gpc_vertex_list * new_contour, int hole)
{
...
void
gpc_polygon_clip(gpc_op op, gpc_polygon * subj, gpc_polygon * clip,
				 gpc_polygon * result)
{
	sb_tree *sbtree = NULL;
		...
		for (edge = aet; edge; edge = edge->next)
		{
			...
			if (exists[CLIP] || exists[SUBJ])
			{
				...
				case GPC_INT:
					...
					tl = (parity[CLIP] ^ (horiz[CLIP] != NH) ^ edge->
						  bundle[BELOW][CLIP])
						&& (parity[SUBJ] ^ (horiz[SUBJ] != NH) ^ edge->
							bundle[BELOW][SUBJ]);
					break;
				case GPC_XOR:
					...
					tr = (parity[CLIP] ^ (horiz[CLIP] != NH))
						^ (parity[SUBJ] ^ (horiz[SUBJ] != NH));
					tl = (parity[CLIP] ^ (horiz[CLIP] != NH) ^ edge->
						  bundle[BELOW][CLIP]) ^ (parity[SUBJ] ^ (horiz[SUBJ]
																  !=
																  NH) ^ edge->
												  bundle[BELOW][SUBJ]);
					break;
		...
		if (scanbeam < sbt_entries)
		{
			...
			for (intersect = it; intersect; intersect = intersect->next)
			{
					...
					&& (e1->bundle[ABOVE][CLIP] || e1->bundle[ABOVE][SUBJ]))
				{
					...
					case GPC_INT:
						...
						bl = (in[CLIP] ^ e1->bundle[ABOVE][CLIP] ^ e0->
							  bundle[ABOVE][CLIP])
							&& (in[SUBJ] ^ e1->bundle[ABOVE][SUBJ] ^ e0->
								bundle[ABOVE][SUBJ]);
						break;
					case GPC_XOR:
						tr = (in[CLIP]) ^ (in[SUBJ]);
			...
			for (edge = aet; edge; edge = next_edge)
			{
				...
				if ((edge->top.y == yt) && succ_edge)
				{
					...
					succ_edge->bundle[BELOW][CLIP] =
						edge->bundle[ABOVE][CLIP];
					succ_edge->bundle[BELOW][SUBJ] =
...
void
gpc_free_tristrip(gpc_tristrip * t)
{
	int s;
...
void
gpc_polygon_to_tristrip(gpc_polygon * s, gpc_tristrip * t)
{
	gpc_polygon c;
...
void
gpc_tristrip_clip(gpc_op op, gpc_polygon * subj, gpc_polygon * clip,
				  gpc_tristrip * result)
{
	sb_tree *sbtree = NULL;
	it_node *it = NULL, *intersect;
	...
	if (result->num_strips > 0)
	{
		...
		for (tn = tlist; tn; tn = tnn)
		{
			...
			if (tn->active > 2)
			{
				...
				MALLOC(result->strip[s].vertex,
					   tn->active * sizeof(gpc_vertex), "tristrip creation",
					   gpc_vertex);
				v = 0;
				if (INVERT_TRISTRIPS)
				{

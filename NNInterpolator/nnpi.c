...
static void
nnpi_add_weight(nnpi * nn, int vertex, double w)
{
	int i;
	...
	{							/* not in the list */
		...
		{
			nn->vertices =
				(int *) realloc(nn->vertices,
								(nn->nallocated + NINC) * sizeof(int));
			nn->weights =
				(double *) realloc(nn->weights,
								   (nn->nallocated + NINC) * sizeof(double));
			nn->nallocated += NINC;
		}
 ...
 */
static void
nnpi_triangle_process(nnpi * nn, point * p, int i)
{
	delaunay *d = nn->d;
	...
	for (j = 0; j < 3; ++j)
	{
		...
		if (!circle_build2(&cs[j], &d->points[v1], &d->points[v2], p))
		{
			...
			else if ((fabs(p2->x - p->x) + fabs(p2->y - p->y)) / c->r <
					 EPS_SAME)
			{
				/*
	...
	for (j = 0; j < 3; ++j)
	{
		...
		int j2 = (j + 2) % 3;
		double det =
			((cs[j1].x - c->x) * (cs[j2].y - c->y) -
			 (cs[j2].x - c->x) * (cs[j1].y - c->y));
...
static int
compare_indexedpoints(const void *pp1, const void *pp2)
{
...
static void
nnpi_getneighbours(nnpi * nn, point * p, int nt, int *tids, int *n,
				   int **nids)
{
	delaunay *d = nn->d;
...
static int
nnpi_neighbours_process(nnpi * nn, point * p, int n, int *nids)
{
	delaunay *d = nn->d;
	int i;
	...
	for (i = 0; i < n; ++i)
	{
		...
		denom1 =
			(p0->x - p->x) * (pp1->y - p->y) - (p0->y - p->y) * (pp1->x -
																 p->x);
		denom2 =
			(p0->x - p->x) * (pm1->y - p->y) - (p0->y - p->y) * (pm1->x -
																 p->x);
		if (denom1 == 0.0)
		{
		...
		nom1 =
			(p0->x - pp1->x) * (pp1->x - p->x) + (p0->y - pp1->y) * (pp1->y -
																	 p->y);
		nom2 =
			(p0->x - pm1->x) * (pm1->x - p->x) + (p0->y - pm1->y) * (pm1->y -
																	 p->y);
...
static int
_nnpi_calculate_weights(nnpi * nn, point * p)
{
	int *tids = NULL;
	int i;
...
static void
nnpi_normalize_weights(nnpi * nn)
{
	int n = nn->nvertices;
	double sum = 0.0;
	int i;
...
void
nnpi_calculate_weights(nnpi * nn, point * p)
{
	point pp;
	int nvertices = 0;
	int *vertices = NULL;
...
static int
cmp_iv(const void *p1, const void *p2)
{
 ...
 */
void
nnpi_interpolate_point(nnpi * nn, point * p)
{
	delaunay *d = nn->d;
	int i;
	...
	if (nn_verbose)
	{
		if (nn_test_vertice == -1)
		{
			...
			{
				ivs =
					(indexedvalue *) malloc(nn->nvertices *
											sizeof(indexedvalue));
 ...
 */
void
nnpi_setwmin(nnpi * nn, double wmin)
{
 ...
 */
nnhpi *
nnhpi_create(delaunay * d, int size)
{
 ...
 */
void
nnhpi_interpolate(nnhpi * nnhpi, point * p)
{
	...
	else
	{
		...
		weights->weights =
			(double *) malloc(sizeof(double) * nnpi->nvertices);
		...
		if (nn_verbose)
		{
			if (nn_test_vertice == -1)
			{
				...
				for (i = 0; i < nnpi->nvertices; ++i)
				{
					fprintf(stderr, "(%d,%.5g)", nnpi->vertices[i],
							nnpi->weights[i]);
 ...
 */
void
nnhpi_modify_data(nnhpi * nnhpi, point * p)
{
 ...
 */
void
nnhpi_setwmin(nnhpi * nn, double wmin)
{
	nn->nnpi->wmin = wmin;
}
...
main(int argc, char *argv[])
{
	...
	{
		long dt =
			1000000.0 * (tv0.tv_sec - tv1.tv_sec) + tv0.tv_usec - tv1.tv_usec;
	...
	if (!nn_verbose)
		printf("    control point: (%f, %f, %f) (expected z = %f)\n",
			   pout[cpi].x, pout[cpi].y, pout[cpi].z, franke(pout[cpi].x,
															 pout[cpi].y));
	...
	{
		long dt =
			1000000.0 * (tv1.tv_sec - tv0.tv_sec) + tv1.tv_usec - tv0.tv_usec;
	...
	if (!nn_verbose)
		printf("    control point: (%f, %f, %f) (expected z = %f)\n",
			   pout[cpi].x, pout[cpi].y, pout[cpi].z,
			   pout[cpi].x * pout[cpi].x - pout[cpi].y * pout[cpi].y);

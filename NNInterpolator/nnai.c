 ...
 */
nnai *
nnai_build(delaunay * d, int n, double *x, double *y)
{
 ...
 */
void
nnai_interpolate(nnai * nn, double *zin, double *zout)
{
	int i;
 ...
 */
void
nnai_setwmin(nnai * nn, double wmin)
{
	nn->wmin = wmin;
}
...
main(int argc, char *argv[])
{
	...
	{
		long dt =
			1000000.0 * (tv2.tv_sec - tv1.tv_sec) + tv2.tv_usec - tv1.tv_usec;
	...
	{
		long dt =
			1000000.0 * (tv0.tv_sec - tv2.tv_sec) + tv0.tv_usec - tv2.tv_usec;
	...
	if (!nn_verbose)
		printf("    control point: (%f, %f, %f) (expected z = %f)\n",
			   xout[cpi], yout[cpi], zout[cpi],
			   xout[cpi] * xout[cpi] - yout[cpi] * yout[cpi]);

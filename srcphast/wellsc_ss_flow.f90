SUBROUTINE wellsc_ss_flow
  ...
  CHARACTER(LEN=80) :: ident_string='$RCSfile: wellsc_ss_flow.f90,v $//$Revision: 2.1 $'
  !     ------------------------------------------------------------------
  !...
  erflg=.FALSE.
  nshut=0
     ...
     IF(iwfss >= 0) THEN
        ...
        DO  ks=1,nks
           ...
           IF(awqm /= 11) THEN
              ...
              utlp=t0
              uclp=0.005_kdp
              udnlp=.5*(denwk(iwel,ks)+denwk(iwel,ks+1))
              uvwlp=uqwm/(udnlp*arwb)
              ! ... Reynolds number for pipe flow
              ren=ABS(uvwlp)*2.*r(1)*udnlp/viscos(uplp,utlp,uclp)
              IF(errexe) RETURN
     ...
     ELSE IF(iwfss < 0) THEN
        ...
        DO  ks=nks,1,-1
           ...
           IF(awqm /= 11) THEN
              ...
              utlm=t0
              uclm=0.005_kdp
              udnlm=.5*(denwk(iwel,ks)+denwk(iwel,ks-1))
              uvwlm=uqwm/(udnlm*arwb)
              ! ... Reynolds number for pipe flow
              ren=ABS(uvwlm)*2.*r(1)*udnlm/viscos(uplm,utlm,uclm)
              IF(errexe) RETURN
              ! ... Calculate the friction factor for well bore (Vennard(1961),chp.9)
              lgren=LOG10(ren)
              ! ... Laminar flow Re<2100

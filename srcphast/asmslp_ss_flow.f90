SUBROUTINE asmslp_ss_flow  
  ...
  INTERFACE
     SUBROUTINE gcgris(ap,bp,ra,rr,ss,xx,w,z,sumfil)
       ...
       REAL(KIND=kdp), DIMENSION(:,0:), INTENT(INOUT) :: bp
...
!
  ...
  CHARACTER(LEN=80) :: ident_string='$RCSfile: asmslp_ss_flow.f90,v $//$Revision: 2.1 $'
  !     ------------------------------------------------------------------
  !...
...
40 CONTINUE
  ...
  IF(JTIME.GT.2) THEN  
     ! ... If DP is too large, abort the p sequence and
...
!!$        WRITE(*,3001) logline1
     ...
     GOTO 40  
  ENDIF
  ! ... Calculate specified P b.c. cell boundary flow rates for current values of DP

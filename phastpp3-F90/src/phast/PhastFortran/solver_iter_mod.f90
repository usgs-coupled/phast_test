...
!!$  WRITE(fuclog,*) 'Current ra-norm: ', ranorm
    ...
    CALL armult_thread(w,rr,xp)
    CALL dbmult_thread(rr,xx_b,xp)
...
50  CONTINUE
    ...
    DO  l=0,nsdr-1
       ...
       apv => ap(:,l)
       bpv => bp(:,l)
       CALL abmult_thread(apv,bpv,xp)
       CALL armult_thread(w,apv,xp)
       CALL dbmult_thread(apv,bpv,xp)
       ...
       IF (l < nsdr-1) THEN
          CALL lsolv(ra,rr,w)
          CALL usolv(ss,ra,w)
          CALL abmult_thread(w,ss,xp)
          CALL armult_thread(z,w,xp)
          CALL dbmult_thread(w,ss,xp)
...
!    enddo
  ...
  SUBROUTINE abmult_thread(x,y,xp)  
    ...
    DO  irn=1,nrn  
       ...
       DO  jc=1,6  
          jcol = ci(jc,irn)  
          IF(jcol > 0) s = s + xp%va(jc,irn)*y(jcol-nrn)  
       END DO
       x(irn) = s  
    END DO
  END SUBROUTINE abmult_thread

  SUBROUTINE armult_thread(x,y,xp)  
    ...
    DO i=1,nbn  
       ...
       DO  j=1,6  
          jcol = ci(j,ii)  
          IF(jcol > 0) s = s + xp%va(j,ii)*y(jcol)  
       END DO
       x(i) = s  
    END DO
  END SUBROUTINE armult_thread

  SUBROUTINE dbmult_thread(x,y,xp)
  ...
  END SUBROUTINE dbmult_thread

  SUBROUTINE formr_thread(ra,xp)
    ...
    !...
    DO  k=1,nbn
       ra(10,k) = xp%va(7,k+nrn)
    END DO
    DO  k=1,nrn
       dd = 1.0D0/xp%va(7,k)
       DO  j=1,6
          xp%va(j,k) = xp%va(j,k)*dd
       END DO
    END DO
    DO  k=1,nrn
       DO  i=1,6
          ...
          IF (irow > 0) THEN
             ...
             nrow = irow-nrn
             DO  j=1,6
                IF (ci(j,k) > 0) ra(mar(i,j),nrow) = ra(mar(i,j),nrow)-dd*xp%va(j,k)
             END DO
          END IF
       END DO
    END DO
  END SUBROUTINE formr_thread
...
!    enddo
  ...
  SUBROUTINE rfact(ra)
    ...
    USE mcs, ONLY: nbn, cirh, cir, mar1
    IMPLICIT NONE
    REAL(KIND=kdp), DIMENSION(:,:), INTENT(INOUT) :: ra
    !
    REAL(KIND=kdp) :: dd
  ...
  SUBROUTINE lsolv(ra,rr,w)
    ...
    USE mcs, ONLY: nbn, cirh, cir, mar1, cirl
    IMPLICIT NONE

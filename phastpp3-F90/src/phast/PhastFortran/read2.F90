SUBROUTINE read2  
  ...
  ALLOCATE (hwt(nxyz),  &
       ...
       STAT = a_err)
  IF (a_err /= 0) THEN  
     PRINT *, "array allocation failed: read2, point 1"  
     STOP  
  ENDIF
  nsa = MAX(ns,1)
  nr = nx
...
120 CONTINUE
  ...
  ALLOCATE (rcppm(1),  &
       abpm(npmz), alphl(npmz), alphth(npmz), alphtv(npmz), tort(npmz), poros(npmz),  &
       ss(npmz),  &
       STAT = a_err)
  IF(a_err /= 0) THEN  
     PRINT * , "array allocation failed: read2, point 2"
     STOP
  ENDIF
  ! ... Allocate the parameter arrays: mcp_m
  ALLOCATE (kx(npmz), ky(npmz), kz(npmz),  &
       kxx(npmz), kyy(npmz), kzz(npmz),  &
       STAT = a_err)
  IF(a_err /= 0) THEN  
     PRINT * , "array allocation failed: read2, point 3"
     STOP
  ENDIF
  ...
  ALLOCATE(i1z(npmz), i2z(npmz), j1z(npmz), j2z(npmz), k1z(npmz), k2z(npmz),  &
       STAT = a_err)
  IF(a_err /= 0) THEN  
     PRINT * , "array allocation failed: read2, point 4"
     STOP
  ENDIF
  DO ipmz = 1, npmz
     i1z(ipmz) = ui1z(ipmz)
...
8008 FORMAT(tr5,2a/(tr5,3(1pg12.4)))
  ...
  IF(nwel > 0) THEN  
     ...
     ALLOCATE (uwid(nxy), uuxw(nxy), uuyw(nxy), uuzwb(nxy), uuzwt(nxy), &
          ...
          STAT = a_err)
     IF (a_err /= 0) THEN  
        PRINT *, "array allocation failed: read2, point 5"  
        STOP  
     ENDIF
     uumwel = 0
     uuwcfl = 0._kdp
...
180  CONTINUE
     ...
     DEALLOCATE (ui1z, ui2z, uj1z, uj2z, uk1z, uk2z, &
          ...
          stat = da_err)
     IF (da_err /= 0) THEN  
        PRINT *, "array deallocation failed; read2"  
        STOP  
     ENDIF
  ENDIF
  ! ... Boundary conditions
  ...
  IF(nsbc > 0) THEN
     ...
     nsbc = nsbc_cells
     nsa = MAX(ns,1)
     ! ... Allocate specified value b.c. arrays: mcb and mcb_m
     ALLOCATE (msbc(nsbc_seg), psbc(nsbc_seg), psbc_n(nsbc_seg),  &
          csbc(nsbc_seg,nsa), indx1_sbc(nsbc_seg), indx2_sbc(nsbc_seg), mxf_sbc(nsbc_seg),  &
          STAT = a_err)
     IF (a_err /= 0) THEN
     ...
     DEALLOCATE (umbc,  &
          stat = da_err)
     IF (da_err /= 0) THEN  
        PRINT *, "array deallocation failed: read2, point 1.1"  
        STOP
     ENDIF
  ENDIF
  ! ... Specified flux b.c.
...
6001    FORMAT (i9.9)
     ...
     DEALLOCATE (umbc, uiface, uabc,  &
          stat = da_err)
     IF (da_err /= 0) THEN  
        PRINT *, "array deallocation failed; read2, point 1.2"  
        STOP
     ENDIF
  ENDIF
  ! ... Aquifer leakage b.c.
  IF(nlbc > 0) THEN  
     ...
     nlbc = nlbc_cells
     nsa = MAX(ns,1)
     ! ... Allocate leakage b.c. arrays: mcb and mcb_m
     ...
     DEALLOCATE (umbc, uiface, uabc, ukbc, ubbbc, uzebc,  &
          stat = da_err)
     IF (da_err /= 0) THEN
        PRINT *, "array deallocation failed: read2, point 1.4"
        STOP
     ENDIF
  ENDIF
  ! ... River b.c. 
  IF(nrbc > 0) THEN  
     ...
     ALLOCATE (umbc(4*nxy), uabc(4*nxy), ukbc(4*nxy), uzebc(4*nxy),  &
          stat = a_err)
     IF (a_err /= 0) THEN  
...
8013    FORMAT(tr1,2i8,4(1pg11.3))
     ...
     nrbc = nrbc_cells
     nsa = MAX(ns,1)
     ! ... Allocate river leakage b.c. arrays: mcb and mcb_m
     ALLOCATE (mrbc(nrbc_seg), arearbc(nrbc_seg),  &
          arbc(nrbc_seg), brbc(nrbc_seg),  &
          krbc(nrbc_seg), bbrbc(nrbc_seg), zerbc(nrbc_seg),  &
          mrseg_bot(nrbc_seg), phirbc(nrbc_seg), phirbc_n(nrbc_seg), &
          denrbc(nrbc_seg), visrbc(nrbc_seg),  &
     ...
     DEALLOCATE (umbc, uabc, ukbc, uzebc,  &
          stat = da_err)
     IF (da_err /= 0) THEN  
        PRINT *, "array deallocation failed: read2, point 1.5"  
        STOP
     ENDIF
  ENDIF
  ! ... Drain b.c. 
  IF(ndbc > 0) THEN  
     ! ... Allocate scratch space for drain data
     ALLOCATE (umbc(4*nxy), uabc(4*nxy), ukbc(4*nxy), uzebc(4*nxy),  &
          stat = a_err)
     IF (a_err /= 0) THEN  
     ...
     ndbc = ndbc_cells
     nsa = MAX(ns,1)
     ! ... Allocate drain b.c. arrays: mcb and mcb_m
     ...
     DEALLOCATE (umbc, uabc, ukbc, uzebc,  &
          stat = da_err)
     IF (da_err /= 0) THEN  
        PRINT *, "array deallocation failed: read2, point 1.6"  
        STOP
     ENDIF
  ENDIF
...
8213       FORMAT(tr1,10i8)
           ...
           DEALLOCATE(uzmic, uziface,  &
                stat = da_err)
           IF (da_err /= 0) THEN  
              PRINT *, "array deallocation failed: read2, flow zones.23.13d"
              STOP
           ENDIF
        END IF
        IF(nsbc_cells > 0) THEN
           ...
           IF(lnk_bc2zon(izn,1)%num_bc > 0) THEN
              ...
              DEALLOCATE (uzmbc,  &
                   stat = da_err)
              IF (da_err /= 0) THEN  
                 PRINT *, "array deallocation failed: read2, flow zones.23.15d"
                 STOP
              ENDIF
           END IF
        END IF
        IF(nfbc_cells > 0) THEN
           ...
           IF(lnk_bc2zon(izn,2)%num_bc > 0) THEN
              ...
              DEALLOCATE (uzmbc,  &
                   stat = da_err)
              IF (da_err /= 0) THEN  
                 PRINT *, "array deallocation failed: read2, flow zones.23.17d"
                 STOP
              ENDIF
           END IF
           IF(fresur) THEN
        ...
        IF(nlbc_cells > 0) THEN
           ...
           IF(lnk_bc2zon(izn,3)%num_bc > 0) THEN
              ...
              DEALLOCATE (uzmbc,  &
                   STAT = da_err)
              IF (da_err /= 0) THEN  
                 PRINT *, "array deallocation failed: read2, flow zones.23.21d"
                 STOP
              ENDIF
           END IF
        END IF
        ...
        IF(ndbc_cells > 0) THEN
           ...
           IF(lnk_bc2zon(izn,4)%num_bc > 0) THEN
              ...
              DEALLOCATE (uzmbc,  &
                   stat = da_err)
              IF (da_err /= 0) THEN  
                 PRINT *, "array deallocation failed: read2, flow zones.23.25d"
                 STOP
              ENDIF
           END IF
        END IF
        IF(nwel > 0) THEN
           ...
           IF(seg_well(izn)%num_wellseg > 0 ) THEN
              ALLOCATE (seg_well(izn)%iwel_no(seg_well(izn)%num_wellseg),  &
                   ...
                   stat = a_err)
              IF (a_err /= 0) THEN
                 PRINT *, "array allocation failed: read2, flow zones.23.28"
                 STOP
              ENDIF

SUBROUTINE XP_coeff_trans_thread(xp)
  ...
  USE mcb, ONLY: ibc, fresur
  USE mcc, ONLY: crosd, cylind, thru, solute
  USE mcg, ONLY: npmz, i1z, i2z, j1z, j2z, k1z, k2z, cellno, arx, ary, arz, &
      nx, nxy, nxyz
        ...
        DO  j=j1z(ipmz),j2z(ipmz)
           DO  i=i1z(ipmz),i2z(ipmz)
              ...
              IF(ibc(m) /= -1 .AND. frac(m) > 0.) THEN
                 ...
                 !$$                 xp%tfx(m)=tx(m)*uden*ufrac/uvis
                 xp%sxx(m)=-xp%tfx(m)*((p(m+1)-p(m)) + uden*gx*(x(i+1)-x(i)))
                 xp%vxx(m)=xp%sxx(m)/(uden*ufrac*arx(m))
20               IF(j == j2z(ipmz) .OR. cylind) GO TO 30
                 ...
                 !$$                 xp%tfy(m)=ty(m)*uden*ufrac/uvis
                 xp%syy(m)=-xp%tfy(m)*((p(mijpk)-p(m)) + uden*gy*(y(j+1)-y(j)))
                 xp%vyy(m)=xp%syy(m)/(uden*ufrac*ary(m))
30               IF(k == k2z(ipmz)) CYCLE
                 ...
                 uden = den0
                 uvis = vis0
                 !$$                 xp%tfz(m)=tz(m)*uden/uvis
                 xp%szz(m)=-xp%tfz(m)*((p(mijkp)-p(m)) + uden*gz*(z(k+1)-z(k)))
                 xp%vzz(m)=xp%szz(m)/(uden*arz(m))
              END IF
           END DO
        END DO
     END DO
              ...
              DO  i=i1z(ipmz),i2z(ipmz)-1
                 ...
                 DO  imm=1,4     ! ... contributions of this element to 4 cell faces
                    ...
                    IF(imm == 1 .OR. imm == 4) THEN
                       u2=.5*(xp%vyy(m)+xp%vyy(mipjk))
                    ELSE IF(imm == 2 .OR. imm == 3) THEN
                       u2=.5*(xp%vyy(mijmk)+xp%vyy(mipjmk))
                    END IF
                    IF(imm == 1 .OR. imm == 2) THEN
                       u3=.5*(xp%vzz(m)+xp%vzz(mipjk))
                    ELSE IF(imm == 3 .OR. imm == 4) THEN
                       u3=.5*(xp%vzz(mijkm)+xp%vzz(mipjkm))
                    END IF
                    ...
                    xp%tsx(m)=xp%tsx(m)+(tdx+tort(ipmz)*dm)*uden*ufrac*updydz/udx
                    xp%tsxy(m)=xp%tsxy(m)+tdxy*uden*ufrac*updydz
                    xp%tsxz(m)=xp%tsxz(m)+tdxz*uden*ufrac*updydz
                 END DO
                 ...
                 DO  imm=1,4
                    ...
                    IF(imm == 1 .OR. imm == 2) THEN
                       u1=.5*(xp%vxx(m)+xp%vxx(mijpk))
                    ELSE IF(imm == 4 .OR. imm == 3) THEN
                       u1=.5*(xp%vxx(mimjk)+xp%vxx(mimjpk))
                    END IF
                    IF(imm == 1 .OR. imm == 4) THEN
                       u3=.5*(xp%vzz(m)+xp%vzz(mijpk))
                    ELSE IF(imm == 3 .OR. imm == 2) THEN
                       u3=.5*(xp%vzz(mijkm)+xp%vzz(mijpkm))
                    END IF
                    uvel=SQRT(u1*u1+u2*u2+u3*u3)
                    tdy=0._kdp
                    tdyx=0._kdp
                    ...
                    ufrac=1._kdp
                    IF(fresur) ufrac=.5*(frac(m)+frac(mijpk))
                    xp%tsy(m)=xp%tsy(m)+(tdy+tort(ipmz)*dm)*uden*ufrac*updxdz/udy
                    xp%tsyx(m)=xp%tsyx(m)+tdyx*uden*ufrac*updxdz
                    xp%tsyz(m)=xp%tsyz(m)+tdyz*uden*ufrac*updxdz
                 END DO
                 ...
                 DO  imm=1,4
                    ...
                    IF(imm == 1 .OR. imm == 4) THEN
                       u1=.5*(xp%vxx(m)+xp%vxx(mijkp))
                    ELSE IF(imm == 2 .OR. imm == 3) THEN
                       u1=.5*(xp%vxx(mimjk)+xp%vxx(mimjkp))
                    END IF
                    IF(imm == 1 .OR. imm == 2) THEN
                       u2=.5*(xp%vyy(m)+xp%vyy(mijkp))
                    ELSE IF(imm == 3 .OR. imm == 4) THEN
                       u2=.5*(xp%vyy(mijmk)+xp%vyy(mijmkp))
                    END IF
                    uvel=SQRT(u1*u1+u2*u2+u3*u3)
                    tdz=0._kdp
                    tdzx=0._kdp
                    ...
                    END IF
                    uden= den0
                    xp%tsz(m)=xp%tsz(m)+(tdz+tort(ipmz)*dm)*uden*updxdy/udz
                    xp%tszx(m)=xp%tszx(m)+tdzx*uden*updxdy
                    xp%tszy(m)=xp%tszy(m)+tdzy*uden*updxdy
                 END DO
              END DO
           END DO
        END DO
        ...
        DO  k=k1z(ipmz),k2z(ipmz)-1
           ...
           DO  i=i1z(ipmz),i2z(ipmz)-1
              ...
              DO  imm=1,2     ! ... contributions of this element to 2 cell face rings
                 ...
                 IF(imm == 1) THEN
                    u3=(1._kdp-ufr)*xp%vzz(m)+ufr*xp%vzz(mipjk)
                 ELSE IF(imm == 2) THEN
                    u3=(1._kdp-ufr)*xp%vzz(mijkm)+ufr*xp%vzz(mipjkm)
                 END IF
  ...
  IF(crosd) THEN
     ierrw = .FALSE.
     DO m=1,nxyz
        !!$  IF(xp%tsx(m) <  0._kdp .or. xp%tsy(m) <  0._kdp .or. xp%tsz(m) <  0._kdp .or.  &
        !!$    xp%tsxy(m) <  0._kdp .or. xp%tsxz(m) <  0._kdp .or. xp%tsyx(m) <  0._kdp .or.  &
        !!$    xp%tsyz(m) <  0._kdp .or. xp%tszx(m) <  0._kdp .or. xp%tszy(m) <  0._kdp) ierr(171) = .true.
        IF(xp%tsx(m) <  0._kdp .OR. xp%tsy(m) <  0._kdp .OR. xp%tsz(m) <  0._kdp .OR.  &
             xp%tsxy(m) <  0._kdp .OR. xp%tsxz(m) <  0._kdp .OR. xp%tsyx(m) <  0._kdp .OR.  &
             xp%tsyz(m) <  0._kdp .OR. xp%tszx(m) <  0._kdp .OR. xp%tszy(m) <  0._kdp) ierrw = .TRUE.
     END DO
     ...
     !**     IF(ierrw) status = RM_ScreenMessage(rm_id, 'WARNING: One or more negative dispersion coefficients computed');
  ENDIF

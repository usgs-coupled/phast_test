...
defined(FALSE)
{
	this->si = new char[::strlen(l_si) + 1];
	::strcpy(this->si, l_si);
}
...
int
cunit::set_input(const char *l_input)
{
	if (!l_input)
		return ERROR;
	...
	this->defined = TRUE;
	this->input = new char[::strlen(l_input) + 1];
	::strcpy(this->input, l_input);
...
void
cunit::define(const char *l_input)
{
	assert(l_input);
	if (!l_input)
		return;

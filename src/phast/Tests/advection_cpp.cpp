...
void advect_cpp(std::vector<double> &c, std::vector<double> bc_conc, int ncomps, int nxyz, int dim);
...
int advection_cpp()
{
	...
	phreeqc_rm.SetPrintChemistryOn(false);
	...
	for (int steps = 0; steps < nsteps; steps++)
	{
		...
		advect_cpp(c, bc_conc, ncomps, nxyz, dim);
...
void
advect_cpp(std::vector<double> &c, std::vector<double> bc_conc, int ncomps, int nxyz, int dim)
{

SUBROUTINE calc_velocity
  ...
  CHARACTER(LEN=80) :: ident_string='$Id: calc_velocity.f90,v 1.1 2013/09/19 20:41:58 klkipp Exp $'
  !     ------------------------------------------------------------------
  ...
  DO l=1,nfbc_cells
     ...
     DO  lc=1,num_bndy_cells
        ...
        IF(mbc == m) THEN               ! ... found the b.c. cell
           DO ls=flux_seg_first(l),flux_seg_last(l)
              ufrac = 1._kdp
  ...
  DO  l=1,nlbc
     ...
     DO  lc=1,num_bndy_cells
        mbc = b_cell(lc)%m_cell
        IF(mbc == m) THEN               ! ... found the b.c. cell
           DO ls=leak_seg_first(l),leak_seg_last(l)
  ...
  DO l=1,nrbc
     m = river_seg_m(l)        ! ... current communicating cell 
  ...
  DO l=1,ndbc
     m = drain_seg_m(l)       ! ... current communicating cell 
     IF(m == 0) CYCLE              ! ... empty cell
     DO  lc=1,num_bndy_cells
        mbc = b_cell(lc)%m_cell
        IF(mbc == m) THEN               ! ... found the b.c. cell
  ...
  DO  l=1,num_bndy_cells
     ...
     DO ibf=1,b_cell(l)%num_faces
        ...
        qface = b_cell(l)%qfbc(ibf)/(den0*frac(m)*b_cell(l)%por_areabc(ibf))
     ...
     IF(cibc(1:1) == '1') THEN
        DO  lc=1,num_bndy_cells
           ...
           IF(mbc == m) THEN               ! ... found the b.c. cell
              ...
              DO ibf=1,b_cell(lc)%num_faces
                 vapp = qface*(qs(ibf)/sumq)/(den0*pa(ibf))
                 ic = b_cell(lc)%face_indx(ibf)
                 IF(fresur .AND. ic == 6) CYCLE
                 IF(ic == 3) THEN
                    vx_node(mbc) = vapp

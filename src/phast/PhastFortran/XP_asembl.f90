SUBROUTINE XP_asembl_thread(xp)
  ...
  CHARACTER(LEN=80) :: ident_string='$Id: XP_asembl.f90,v 1.1 2013/09/19 20:41:58 klkipp Exp $'
...
!!$        IF(szzp < 0.) wtpz = 1._kdp-wtpz
     ...
     IF(mimjk > 0) THEN
        ...
        xp%va(icxm,ma) = -fdtmth*utxm
        xp%va(7,ma) = xp%va(7,ma) + fdtmth*(tsxm-wtmx*sxxm)
     END IF
     IF(mipjk > 0) THEN
        ...
        utxp = tsxp - wtpx*sxxp
        xp%va(icxp,ma) = -fdtmth*utxp
        xp%va(7,ma) = xp%va(7,ma) + fdtmth*(tsxp+(1._kdp-wtpx)*sxxp)
     END IF
     ! ... Y-direction
     IF(mijmk > 0) THEN
        ...
        xp%va(icym,ma) = -fdtmth*utym
        xp%va(7,ma) = xp%va(7,ma)+fdtmth*(tsym-wtmy*syym)
     END IF
     IF(mijpk > 0) THEN
        ...
        utyp = tsyp - wtpy*syyp
        xp%va(icyp,ma) = -fdtmth*utyp
        xp%va(7,ma) = xp%va(7,ma)+fdtmth*(tsyp+(1._kdp-wtpy)*syyp)
     END IF
     ! ... Z-direction
     IF(mijkm > 0) THEN
        ...
        xp%va(iczm,ma) = -fdtmth*utzm
        xp%va(7,ma) = xp%va(7,ma)+fdtmth*(tszm-wtmz*szzm)
     END IF
     IF(mijkp > 0) THEN
        ...
        utzp = tszp-wtpz*szzp
        xp%va(iczp,ma) = -fdtmth*utzp
        xp%va(7,ma) = xp%va(7,ma)+fdtmth*(tszp+(1._kdp-wtpz)*szzp)
     END IF
     xp%rhs(ma) = xp%rs1(m)+fdtmth*ur1-xp%c13*dp(m)-xp%csp*dpmkm-xp%esp*dpmkp
  END DO
  DEALLOCATE (xp%rs1,  &
       STAT  =  da_err)
  IF (da_err /= 0) THEN  
...
END SUBROUTINE XP_asembl_thread

...
!!$  USE f_units
  ...
  CHARACTER(LEN=80) :: ident_string='$Id: dealloc_arr_worker.F90,v 1.3 2011/01/29 00:18:54 klkipp Exp klkipp $'
  !     ------------------------------------------------------------------
  !...
...
!!$         c, &
  ...
  IF (da_err /= 0) THEN
     ...
     STOP
  ENDIF
  ! ... Deallocate dependent variable arrays: mcv_w
  ! ...      component arrays
!!$  DEALLOCATE (dc, c_w,  &
!!$       STAT = da_err)
!!$  IF (da_err /= 0) THEN
!!$     PRINT *, "Array deallocation failed: dealloc_arr_worker, init1_trans, point 5"  
!!$     STOP
!!$  ENDIF
...
!!$       STAT = da_err)
!!$  IF (da_err /= 0) THEN
!!$     PRINT *, "Array deallocation failed: dealloc_arr_worker, init1_trans, point 6"  
!!$     STOP
!!$  ENDIF
  ...
  IF (xp_group) THEN
     ...
     IF(nwel > 0) THEN
        ...
        IF (da_err /= 0) THEN  
           ...
           STOP  
        ENDIF
        ! ... Deallocate more well arrays: mcw_w
!!$     DEALLOCATE (qslyr, qsw,  &
!!$          cwk, cwkt, cwkts,  &
!!$          stfwi, sthwi, stswi,  &
!!$          stfwp, sthwp, stswp,  &
!!$          STAT = da_err)
!!$     IF (da_err /= 0) THEN  
!!$        PRINT *, "Array deallocation failed: dealloc_arr_worker, init2.1_trans, number 8"  
!!$        STOP  
!!$     ENDIF
     END IF
     IF(nsbc > 0) THEN  
...
!!$          STAT = da_err)
!!$     IF (da_err /= 0) THEN  
!!$        PRINT *, "array deallocation failed: dealloc_arr_worker, init2.1_trans, number 11"  
!!$        STOP
!!$     ENDIF
        ! ... Deallocate difference equations arrays: mcm_w
        !     DEALLOCATE (xp%vassbc, xp%rhssbc,  &
        !          STAT = da_err)
        !     IF (da_err /= 0) THEN  
        !        PRINT *, "array deallocation failed: dealloc_arr_worker, init2.1_trans, number 12, svbc"  
        !        STOP
        !     ENDIF
     END IF
     IF(nfbc > 0) THEN

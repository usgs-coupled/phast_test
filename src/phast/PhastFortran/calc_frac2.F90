subroutine calc_frac2()
  ...
  USE PhreeqcRM
  IMPLICIT NONE
  REAL(kind=kdp), allocatable, dimension(:) :: dzfsdt_t, frac_t, p_t, vmask_t, zfs_t, zfsn_t
  INTEGER(kind=kdp), allocatable, dimension(:) :: mfsbc_t
  INTEGER :: status
  ...
  allocate (dzfsdt_t(nxy), frac_t(nxyz), p_t(nxyz), vmask_t(nxyz), zfs_t(nxy), zfsn_t(nxy), mfsbc_t(nxy))
  dzfsdt_t = dzfsdt
  frac_t = frac
  p_t = p
  vmask_t = vmask
  ...
  IF(.NOT.steady_flow) THEN          ! ... skip this if steady state flow
      ...
      IF(fresur) THEN
          ...
          ! ...      per time step
          DO  mt=1,nxy
              m=mfsbc_t(mt)          ! ... w.t. cell at time n
              ...
              DO  l=1,nsbc
                  ...
                  IF(k == 1) THEN
                      ...
                      IF(p_t(m) > 0._kdp) THEN
                          ...
                          frac_t(m) = 2.*(zfsl-z0)/(zp1-z0)
                          frac_t(m) = MIN(1._kdp,frac_t(m))
                          vmask_t(m) = 1
                      ELSE
                          frac_t(m) = 0._kdp
                          vmask_t(m) = 0
                  ...
                  ELSE              ! ... Intermediate plane
                      ...
                      ELSE                      ! ... True intermediate plane
                          IF(p_t(m) > 0._kdp) THEN
                              ...
                              zp1=z(k+1)
                              zfsl = up0/(den0*gz) + z0     ! Hydrostatic
                              frac_t(m) = (2.*zfsl-(z0+zm1))/(zp1-zm1)
                              frac_t(m) = MIN(1._kdp,frac_t(m))
                              vmask_t(m) = 1
                          ELSE
                              up0=p_t(m)
          ...
          DO  mt=1,nxy
              ...
              IF(frac_t(m) > 1._kdp) THEN
                  ...
                  frac_t(m+nxy)=(frac_t(m)-1._kdp)*pv(m)/pv(m+nxy)
                  !IF(k+1 == nz .OR. ibc(m+2*nxy) == -1) THEN
                  IF(k+1 == nz) THEN
                      zfs_t(mt)=.5_kdp*(z1+z0+frac_t(m+nxy)*(z1-z0))     ! half cell thickness
                  ELSE IF (ibc(m+2*nxy) == -1) THEN
                      zfs_t(mt)=.5_kdp*(z1+z0+frac_t(m+nxy)*(z1-z0))     ! half cell thickness
                  ELSE
                      z2=z(k+2)
                      zfs_t(mt)=.5_kdp*(z1+z0+frac_t(m+nxy)*(z2-z0))     ! full cell thickness
                  END IF
                  vmask_t(m+nxy) = 0
                  IF(zfs_t(mt) >= z1) vmask_t(m+nxy) = 1
                  p_t(m+nxy)=up0*(zfs_t(mt)-z1)/(zfs_t(mt)-z0)
                  hdprnt(m+nxy)=z1+p_t(m+nxy)/(den0*gz)
                  !$$           den(m+nxy)=den(m)
                  !$$           vis(m+nxy)=vis(m)
                  IF(solute) THEN
                      DO  is=1,ns
                      ...
                      END DO
                  END IF
                  ! ... Now set saturated cell to frac_t of 1.
                  frac_t(m) = 1._kdp
                  vmask_t(m) = 1
              END IF
          END DO
          ...
          ! ... also set all frac_t to one for cells below the f.s. cell
          DO  mt=1,nxy
              ierrw = .FALSE.
              m0 = mfsbc_t(mt)
              mfsbc_t(mt) = 0
...
6001              FORMAT(i9.9)
                  IF(cibc(1:1) /= '1') p_t(m1kp)=pmfs-denmfs*gz*(z(kfs+k)-zmfs)
                  IF(solute) THEN
                      DO  is=1,ns

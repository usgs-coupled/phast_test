...
2005    FORMAT(//tr30,4A)
        CALL printar(1,x,ibc,fu(ifu),cnvli,12,nx)
        WRITE(fu(ifu),2005) 'Y-Direction Node Coordinates   (',TRIM(unitl),')'
        CALL printar(1,y,ibc,fu(ifu),cnvli,12,ny)
     ELSE IF(cylind) THEN
...
2013 FORMAT(/tr30,a)
     ...
     ALLOCATE (lprnt4(nele),  &
          stat = a_err)
     IF (a_err /= 0) THEN  
        PRINT *, "Array allocation failed: write2_1, number 2"  
        STOP  
     ENDIF
     lprnt4 = 0
     aprnt1 = 0.
...
!!$        CALL printar(1,alphl,ibc,fulp,cnvli,24,npmz)
!!$        WRITE(fulp,2014) 'Transverse Dispersivity; Horizontal   (',TRIM(unitl),')'
!!$        CALL printar(1,alphth,ibc,fulp,cnvli,24,npmz)
!!$        WRITE(fulp,2014) 'Transverse Dispersivity; Vertical   (',TRIM(unitl),')'
!!$        CALL printar(1,alphtv,ibc,fulp,cnvli,24,npmz)
...
2014    FORMAT(/tr30,8A)
        ...
        CALL printar(2,aprnt3,lprnt4,fulp,cnvli,24,-111)
        WRITE(fulp,2014) 'Tortuosity by Element (',TRIM(unitl),')'
        CALL printar(2,aprnt4,lprnt4,fulp,cnvli,24,-111)        
     END IF
     IF(solute) THEN
...
2017 FORMAT(/tr25,a60,a,f10.1,tr2,3A/tr25,a60,a,f10.1,tr2,3A)
     ...
     DEALLOCATE (lprnt4,  &
          stat = da_err)
     IF (da_err /= 0) THEN  
        PRINT *, "Array deallocation failed: write2_1, number 3"  
     ENDIF
  END IF
  IF(prt_kd) THEN
...
60   IF(visfac > 0.) WRITE(fulp,2023)
...
!          stat = da_err)
!     IF (da_err /= 0) THEN  
!        PRINT *, "Array deallocation failed: write2_1, number 4"  
!        STOP  
!     ENDIF
  END IF
...
!!$        CALL printar(2,uvaifc,lprnt1,fulp,cnv,24,000)
!!$     END IF
  END IF
!!$300 CONTINUE
...
5060    FORMAT(a65,a,1pe8.1)
  ...
  IF(num_flo_zones > 0) THEN
     ...
     CALL printar(2,aprnt1,lprnt1,fuzf,cnv,10,000)
  END IF
...
!!$  END IF
  DEALLOCATE (aprnt5, ss,  &
       stat = da_err)
  IF (da_err /= 0) THEN  
     PRINT *, "Array deallocation failed: write2_1, number 5"  
  ENDIF
...
!$$    nval = MIN(SIZE(xvalt), SIZE(irngt))
    ...
    ALLOCATE (jwrkt(1:nval),  &
         stat = a_err)
    IF (a_err /= 0) THEN  
       PRINT *, "Array allocation failed: merge_ref, number 1"  
       STOP
    ENDIF
    lmtnc = 2
    lmtna = 2
    ...
    DEALLOCATE (jwrkt,  &
         stat = da_err)
    IF (da_err /= 0) THEN  
       PRINT *, "Array deallocation failed: merge_ref, number d1"
       STOP
    ENDIF
  END SUBROUTINE merge_ref

  SUBROUTINE mptoiwks(m,i,k,ni)
    ! ... Returns the index (I,K) of the point on the wellbore with index M.
    !
    IMPLICIT NONE

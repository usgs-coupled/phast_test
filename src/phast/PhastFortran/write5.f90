SUBROUTINE write5
  ...
  INTEGER :: a_err, da_err, i, ic, ifmt, iis, indx,  &
       ip, is1, is2, iw1p, iw2p, iwel, iwfss, iwpp, izn, j, jprptc,  &
...
!!$  !         IFMT=12
!!$  !         IF(EEUNIT) IFMT=13
!!$  !         CALL PRINTAR(2,DEN,LPRNT1,FUD,CNVDI,IFMT,000)
...
!!$  !         IFMT=24
!!$  !         IF(EEUNIT) IFMT=15
!!$  !         CALL PRINTAR(2,VIS,LPRNT1,FUVS,CNVVSI,IFMT,000)
!!$  !      ENDIF
  IF(prgfb) THEN
        ...
        IF (solute) THEN
           DO  is=1,ns
              ...
              IF(prthd3) THEN
                 ...
                 CALL printar(2,aprnt3,lprnt3,fubcf,cnvmfi,24,000)
              END IF
           END DO
        ENDIF
        IF (solute) THEN
...
2043    FORMAT(//tr25,a/tr25,a)
        ...
        DO  lc=1,nfbc_cells
           ...
           IF(fresur) THEN
              ...
              END IF
           END IF
           IF (m > 0) THEN          ! ... Skip over columns of dry cells
              lprnt1(m) = 1
              !$$              prthd=.TRUE.
              aprnt1(m) = qffbc(lc)
           END IF
        ...
        IF (solute) THEN
           DO  iis=1,ns
              ...
              IF(prthd) THEN
                 ...
                 CALL printar(2,aprnt3,lprnt1,fubcf,cnvmfi,24,000)
              END IF
           END DO
        ENDIF
        IF (solute) THEN
        ...
        IF (solute) THEN
           DO  iis=1,ns
              ...
              WRITE(fubcf,2042) 'River Leakage B.C.: Associated Advective Solute   ('  &
                   //unitm//'/'//TRIM(unittm)//')'
              WRITE(fubcf,2042) 'Component: ', comp_name(iis)
              CALL printar(2,aprnt3,lprnt4,fubcf,cnvmfi,24,000)
           END DO
        END IF
     END IF
     IF(ndbc > 0) THEN
...
!!$           CALL printar(2,aprnt2,lprnt1,fubcf,cnvhfi,24,000)
!!$        END IF
!!$        IF(solute) THEN
...
!!$       timprp, timprc, timprcphrq, timprfchem, timprslm, timprtem, timprvel, timprwel)
  DEALLOCATE (lprnt3, lprnt4,  &
       stat = da_err)
  IF (da_err /= 0) THEN  
     PRINT *, "Array deallocation failed: write5"  
  ENDIF

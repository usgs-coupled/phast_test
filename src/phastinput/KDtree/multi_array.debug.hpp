...
public:
  ...
  multi_array& resize(const detail::multi_array
    ...
    if (&view_old != &view_new) {
      // make sure the dimensions agree
      BOOST_ASSERT(view_old.num_dimensions() == view_new.num_dimensions());
      BOOST_ASSERT(std::equal(view_old.shape(),
                              view_old.shape()+view_new.num_dimensions(),
                              view_new.shape()));
	  multi_array < T, NumDims >::const_iterator other_it = view_old.begin();
	  multi_array < T, NumDims >::iterator this_it = view_new.begin();
	  for (; this_it != view_old.end(); ++this_it, ++other_it)
		  *this_it = *other_it;
    }
#else
    view_new = view_old;

...
class PHAST_polygon
{
  public:
	PHAST_polygon(void);
	  PHAST_polygon(gpc_polygon * poly,
					PHAST_Transform::COORDINATE_SYSTEM cs);
	  PHAST_polygon(const std::vector < Point > &pts,
					PHAST_Transform::COORDINATE_SYSTEM cs);
  ...
  public:
	...
	bool Line_intersect(Point lp1, Point lp2,
						std::vector < Point > &intersect_pts);
	  std::vector < Point > &Get_points()
	{
		return this->pts;
	};
	std::vector < std::vector < Point >::iterator > &Get_begin()
	{
		return this->begin;
	};
	std::vector < std::vector < Point >::iterator > &Get_end()
	{
		return this->end;
	};
...
bool Point_in_simple_polygon(Point p, std::vector < Point >::iterator begin,
							 std::vector < Point >::iterator end);
bool Line_intersect_simple_polygon(Point lp1, Point lp2,

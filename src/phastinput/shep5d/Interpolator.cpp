...
//   IW = new int[LIW];
   ...
   WS = new double[LWS+1];     // float work space array >= NT**2
   if(!WS){
   ...
   XMIN = new double[M+1];     // array of min. nodal coordinates
   if(!XMIN){
     std::cerr << "XMIN array not allocated\n";
     exit(1);
    }

   DX = new double[M+1];       // array of min. cell dimensions
   if(!DX){
...
**************************************************************************/
double
**Interpolator::QSMTST3(int *Axes_Dim,double Axes_Ranges[][3],char ans) {

   int i,j,                   // loop indices
       NWMAX;                 // max. value for NW, MIN0(50,N-1)

   double step,                // length between tick marks on axis
  **Axes_Ticks;               // array containing tick mark values on axes
   ...
   if((N < NT)||(N > NMAX)){
     std::cout << "       *** ERROR -- N = " << N << "  MAXIMUM VALUE = "; 
...
//cout << W[i] << endl;
   ...
   std::cout << "N = " << N << " NWMAX = " << NWMAX << std::endl;
   std::cout << "\n\n";

   // obtain values for input parameters for NQ, NW, and NR - check if legit.
   NQ = 0;
   while((NQ < NT-1)||(NQ > NWMAX)) {
     std::cout << "QSMTST3:  N = " << N << std::endl;
     std::cout << "Specify the number of nodes NQ for the least squares fit\n";
     std::cout << "NQ = 17 is recommended.  9 < NQ < Min(50,N-1) or NQ = 0 to\n";
     std::cout << "terminate.\n";
     scanf("%d",&NQ);
   ...
   NW=0;
   while((1 > NW)||(NW > NWMAX)){
     std::cout << "Specify the number of nodes NW for the weights\n";
     std::cout << "NW = 32 is recommended.  1 < NW < Min(50,N-1)\n";
     scanf("%d",&NW);
   ...
   NR=0;
   while((NR < 1)||(NR > NRMAX)){
     std::cout << "Specify the number of rows, columns, and planes NR in the\n";
     std::cout << "uniform grid of cells used to locate nearest neighbors.\n";
     std::cout << "NR = (N/3)**(1/M) is recommended. 1 < NR < NRMAX\n";
     scanf("%d",&NR);
     std::cout << "NR = " << NR << std::endl;
    }

   //allocate Axes_Ticks array
   Axes_Ticks = new double*[M+1];
   if(!Axes_Ticks){
     std::cerr << "Axes_Ticks array not allocated\n";
     exit(1);
    }

   for(i=1;i<M+1;i++){
     Axes_Ticks[i] = new double[Axes_Dim[i]+1];
     if(!Axes_Ticks[i]){
       std::cerr << "Axes_Ticks array not allocated\n";
       exit(1);
      }
              ...
              do{            //do-while #7
                ...
                if(NEQ != LMAX){  //else break out of #7 and GO TO top of #4
                  if(NEQ < LNP){
                    ...
                    RS = 0.0;
                    for(J=1;J<M+1;J++){ //#8
//                      RS += (double)pow((double)(X[J][NP]-X[J][K]),(double)2);
                    RS += (double)(X[J][NP]-X[J][K])*(double)(X[J][NP]-X[J][K]);
                     }
                   } 
                  else{
                    NEQ = LNP;
...
Interpolator::QSMVAL(double *P){
  ...
  double  DS,         // Used in determining weights
         QK,         // Value of Q(K)(P)
   ...
   do{                   //do-while #2
     ...
     if(K != 0){         // else break out to #8
       ...
       do{ //#3
         DS = 0.0;
         for(I=1;I<M+1;I++){
//           DS += (double)pow((double)(P[I]-X[I][K]),(double)2);
           DS += (double)(P[I]-X[I][K])*(double)(P[I]-X[I][K]);
          }
         RS = RSQ[K];
         if(RS > DS){  // GOTO #7
          if(DS != 0){  // GOTO #10
            RDS = RS * DS;
            RD = (double)sqrt((double)RDS);
            WW = (RS+DS-RD-RD)/RDS;
            SW += WW;
...
//                  RRSQ += (double)pow((double)(X[J][K]-X[J][L]),(double)2);
...
**************************************************************************/
void
Interpolator::GIVENS(double *A, double *B, double &C, double &S){
   double AA,       // Local copy of A 
         BB,       // Local copy of B
   ...
   if(abs(AA) > abs(BB)){
     U = AA + AA;
     V = BB/U;
     R = (double)sqrt((double)(0.25+V*V)) * U;
     C = AA/R;
     S = V * (C+C);
     ...
     *A = R;
     ...
     if(BB == 0.0){ // #1
       ...
       // Store R in A
       *A = (double)sqrt((double)(0.25+V*V)) * U;
       S = BB/(*A);
       C = V * (S+S);
...
Interpolator::SETUPM(int K,double FK,int NP,double S1,double S2,double R,int ROW){
  ...
  double   D,         // Distance between nodes L and K
    ...
    *WS_PTR;         // Pointer into array WS
    ...
    if(M >= 1){   // else "GO TO 4"
      ...
      D = 0.0;
      for(I=1;I<M+1;I++){
//        D += (double)pow((double)(X[I][NP]-X[I][K]),(double)2);
        D += (double)(X[I][NP]-X[I][K])*(double)(X[I][NP]-X[I][K]);
       }  
      D = (double)sqrt((double)D);
...
Interpolator::STOREM(int &IER){
 ...
 double RNR;         // Local variable for NR
...
****************************************************************************/
void
Interpolator::get_node_coords(double *PP, int K){
...
****************************************************************************/
void
Interpolator::TFUN3(int KF,int K,double *PP,int iflag, int Fflag, double *F,double DUM,double DUM1,double DUM2){

  double *tmp,     // temp variable that holds function values
...
//  cout << X << " " << Y << " " << Z << endl;
  ...
  switch (KF){
   ...
   case 1:  // Exponential
    *tmp = 0.75*(double)exp(-(pow((double)(9*X-2),2) + pow((double)(9*Y-2),2) +                 pow((double)(9*Z-2),2))/4) +
           0.75*(double)exp(-(pow((double)(9*X+1),2))/49 - (9*Y+1)/10 - 
                 (9*Z+1)/10) +
           0.50*(double)exp(-(pow((double)(9*X-7),2) + pow((double)(9*Y-3),2) +
                 (9*Z+1))/4) - 
           0.20*(double)exp(-pow((double)(9*X-4),2) - pow((double)(9*Y-7),2) - 
                 pow((double)(9*Z-5),2));
    if(iflag != 1) {
      return;
     }
   ...
   case 2:   // Cliff 
    *tmp = ((double)tanh((double)(9*(Z-X-Y)))+1)/9;
    if(iflag != 1) {
      return;
     }
    break;
   ...
   case 3:  // Saddle
    *tmp = (1.25 + (double)cos((double)(5.4*Y)))*(double)cos((double)(6*Z)) /
           (6 + 6*(double)pow((double)(3*X-1),2));
    if(iflag != 1) {
      return;
     }
    break;
   ...
   case 4:  // Gentle
    *tmp = (double)exp(-5.0625*(pow((double)(X-0.5),2)+pow((double)(Y-0.5),2)
   ...
   case 5:  // Steep
    *tmp = (double)exp(-20.25*(pow((double)(X-0.5),2)+pow((double)(Y-0.5),2)
           + pow((double)(Z-0.5),2)))/3;
    if(iflag != 1){
      return;
     }

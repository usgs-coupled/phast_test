 ...
 */
hashtable *
ht_create(int size, ht_keycp cp, ht_keyeq eq, ht_key2hash hash)
{
 ...
 */
void *
ht_insert(hashtable * table, void *key, void *data)
{
 ...
 */
void *
ht_find(hashtable * table, void *key)
{
	unsigned int val = table->hash(key) % table->size;
	ht_bucket *bucket;
 ...
 */
void *
ht_delete(hashtable * table, void *key)
{
	unsigned int val = table->hash(key) % table->size;
	ht_bucket *prev;
	ht_bucket *bucket;
	 ...
	 * contains. 
	 */
	for (prev = NULL, bucket = (table->table)[val]; bucket != NULL;
 ...
 */
void
ht_process(hashtable * table, void (*func) (void *))
{
	int i;
...
static int
d2eq(void *key1, void *key2)
{
	return (((double *) key1)[0] == ((double *) key2)[0])
...
int
ht_getnentries(hashtable * table)
{
	return table->n;
}

int
ht_getsize(hashtable * table)
{
	return table->size;
}

int
ht_getnfilled(hashtable * table)
{
	return table->nhash;
}
...
main()
{
	...
	printf("  inserting %d elements with deep copy of each data string...",
		   size);
	for (i = 0; i < size; ++i)
	{
		char key[BUFSIZE];

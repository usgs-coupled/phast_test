...
static void
tio_init(struct triangulateio *tio)
{
	tio->pointlist = NULL;
...
static void
tio_destroy(struct triangulateio *tio)
{
...
static void
tio2delaunay(struct triangulateio *tio_out, delaunay * d)
{
	int i, j;
	...
	assert(tio_out->pointlist[2 * d->npoints - 2] ==
		   d->points[d->npoints - 1].x
		   && tio_out->pointlist[2 * d->npoints - 1] ==
		   d->points[d->npoints - 1].y);
	...
	if (d->ntriangles > 0)
	{
		...
		d->neighbours =
			(triangle_neighbours *) malloc(d->ntriangles *
										   sizeof(triangle_neighbours));
	...
	if (d->ntriangles > 0)
	{
		for (i = 0; i < d->npoints; ++i)
		{
			if (d->n_point_triangles[i] > 0)
				d->point_triangles[i] =
					(int *) malloc(d->n_point_triangles[i] * sizeof(int));
			else
 ...
 */
delaunay *
delaunay_build(int np, point points[], int ns, int segments[], int nh,
			   double holes[])
{
 ...
 */
static int
onrightside(point * p, point * p0, point * p1)
{
 ...
 */
int
delaunay_xytoi(delaunay * d, point * p, int id)
{
	triangle *t;
	int i;
	...
	do
	{
		for (i = 0; i < 3; ++i)
		{
			...
			if (onrightside
				(p, &d->points[t->vids[i]], &d->points[t->vids[i1]]))
			{
...
static void
delaunay_addflag(delaunay * d, int i)
{
	if (d->nflags == d->nflagsallocated)
	{
		d->nflagsallocated += N_FLAGS_INC;
		d->flagids =
			(int *) realloc(d->flagids, d->nflagsallocated * sizeof(int));
	}
...
static void
delaunay_resetflags(delaunay * d)
{
	int i;
 ...
 */
void
delaunay_circles_find(delaunay * d, point * p, int *n, int **out)
{
	/*

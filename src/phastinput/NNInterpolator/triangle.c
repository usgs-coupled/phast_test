...
#else /* not ANSI_DECLARATORS */
int
triunsuitable(triorg, tridest, triapex, area)
...
#ifdef ANSI_DECLARATORS
void
parsecommandline(int argc, char **argv, struct behavior *b)
#else /* not ANSI_DECLARATORS */
void
			...
			for (j = STARTINDEX; argv[i][j] != '\0'; j++)
			{
				...
				if (argv[i][j] == 'q')
				{
						...
						|| (argv[i][j + 1] == '.'))
					{
						...
						workstring[k] = '\0';
						b->minangle =
							(REAL) strtod(workstring, (char **) NULL);
					}
					else
					{
						b->minangle = 20.0;
...
#ifdef ANSI_DECLARATORS
void
printtriangle(struct mesh *m, struct behavior *b, struct otri *t)
#else /* not ANSI_DECLARATORS */
void
printtriangle(m, b, t)
	 struct mesh *m;
...
#ifdef ANSI_DECLARATORS
void
printsubseg(struct mesh *m, struct behavior *b, struct osub *s)
#else /* not ANSI_DECLARATORS */
void
printsubseg(m, b, s)
	 struct mesh *m;
...
#else /* not ANSI_DECLARATORS */
void
poolinit(pool, bytecount, itemcount, wtype, alignment)
	 struct memorypool *pool;
	 int bytecount;
	 int itemcount;
	 enum wordtype wtype;
...

{
	...
	wordsize =
		(pool->itemwordtype == POINTER) ? sizeof(VOID *) : sizeof(REAL);
	...
	else
	{
		...
		if (pool->unallocateditems == 0)
		{
			...
			if (*(pool->nowblock) == (VOID *) NULL)
			{
				...
				newblock =
					(VOID **) trimalloc(pool->itemsperblock *
										pool->itembytes + sizeof(VOID *) +
										pool->alignbytes);
		...
		{
			pool->nextitem =
				(VOID *) ((VOID **) pool->nextitem + pool->itemwords);
		}
		else
		{
			pool->nextitem =
				(VOID *) ((REAL *) pool->nextitem + pool->itemwords);
		}
...

{
	...
	{
		pool->pathitem =
			(VOID *) ((VOID **) pool->pathitem + pool->itemwords);
	}
	else
	{
...
#ifdef ANSI_DECLARATORS
void
dummyinit(struct mesh *m, struct behavior *b, int trianglewords,
		  int subsegwords)
#else /* not ANSI_DECLARATORS */
void
dummyinit(m, b, trianglewords, subsegwords)
	 struct mesh *m;
	 struct behavior *b;
	 int trianglewords;
	 int subsegwords;
...

{
	...
	m->dummytribase =
		(triangle *) trimalloc(trianglewords * sizeof(triangle) +
...
#ifdef ANSI_DECLARATORS
void
initializevertexpool(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
void
...
#ifdef ANSI_DECLARATORS
void
initializetrisubpools(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
void
...

{
	...
	trisize =
		((b->order + 1) * (b->order + 2) / 2 +
		 (m->highorderindex - 3)) * sizeof(triangle);
	...
	if (b->usesegments)
	{
		...
		poolinit(&m->subsegs, 6 * sizeof(triangle) + sizeof(int),
...
#ifdef ANSI_DECLARATORS
void
triangledealloc(struct mesh *m, triangle * dyingtriangle)
#else /* not ANSI_DECLARATORS */
void
triangledealloc(m, dyingtriangle)
	 struct mesh *m;
...
#ifdef ANSI_DECLARATORS
triangle *
triangletraverse(struct mesh *m)
#else /* not ANSI_DECLARATORS */
triangle *
triangletraverse(m)
	 struct mesh *m;
...
#ifdef ANSI_DECLARATORS
void
subsegdealloc(struct mesh *m, subseg * dyingsubseg)
#else /* not ANSI_DECLARATORS */
void
subsegdealloc(m, dyingsubseg)
	 struct mesh *m;
	 subseg *dyingsubseg;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void
vertexdealloc(struct mesh *m, vertex dyingvertex)
#else /* not ANSI_DECLARATORS */
void
vertexdealloc(m, dyingvertex)
	 struct mesh *m;
	 vertex dyingvertex;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void
badsubsegdealloc(struct mesh *m, struct badsubseg *dyingseg)
#else /* not ANSI_DECLARATORS */
void
badsubsegdealloc(m, dyingseg)
	 struct mesh *m;
...
#ifdef ANSI_DECLARATORS
struct badsubseg *
badsubsegtraverse(struct mesh *m)
#else /* not ANSI_DECLARATORS */
struct badsubseg *
badsubsegtraverse(m)
	 struct mesh *m;
...
#ifdef ANSI_DECLARATORS
vertex
getvertex(struct mesh * m, struct behavior * b, int number)
#else /* not ANSI_DECLARATORS */
vertex
getvertex(m, b, number)
	 struct mesh *m;
	 struct behavior *b;
	 int number;
#endif /* not ANSI_DECLARATORS */

{
	...
	foundvertex =
		(vertex) (alignptr + (unsigned long) m->vertices.alignbytes -
...
#ifdef ANSI_DECLARATORS
void
triangledeinit(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
void
triangledeinit(m, b)
	 struct mesh *m;
...
#ifdef ANSI_DECLARATORS
void
maketriangle(struct mesh *m, struct behavior *b, struct otri *newotri)
#else /* not ANSI_DECLARATORS */
void
maketriangle(m, b, newotri)
	 struct mesh *m;
	 struct behavior *b;
	 struct otri *newotri;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void
makesubseg(struct mesh *m, struct osub *newsubseg)
#else /* not ANSI_DECLARATORS */
void
...
#else /* not ANSI_DECLARATORS */
REAL
counterclockwiseadapt(pa, pb, pc, detsum)
	 vertex pa;
	 vertex pb;
	 vertex pc;
	 REAL detsum;
...
#ifdef ANSI_DECLARATORS
REAL
counterclockwise(struct mesh * m, struct behavior * b,
				 vertex pa, vertex pb, vertex pc)
#else /* not ANSI_DECLARATORS */
REAL
counterclockwise(m, b, pa, pb, pc)
	 struct mesh *m;
	 struct behavior *b;
	 vertex pa;
	 vertex pb;
	...
	if (adxtail != 0.0)
	{
		...
		temp48len =
			fast_expansion_sum_zeroelim(temp16clen, temp16c, temp32alen,
										temp32a, temp48);
		finlength =
			fast_expansion_sum_zeroelim(finlength, finnow, temp48len, temp48,
										finother);
		finswap = finnow;
		finnow = finother;
		finother = finswap;
		...
		if (adxtail != 0.0)
		{
			...
			temp32alen =
				scale_expansion_zeroelim(axtbctlen, axtbct, 2.0 * adx,
										 temp32a);
			temp48len =
				fast_expansion_sum_zeroelim(temp16alen, temp16a, temp32alen,
											temp32a, temp48);
			finlength =
...

{
	...
	det = alift * (bdxcdy - cdxbdy)
		+ blift * (cdxady - adxcdy) + clift * (adxbdy - bdxady);
...

{
	...
	if ((adxtail == 0.0) && (bdxtail == 0.0) && (cdxtail == 0.0) &&
		(adytail == 0.0) && (bdytail == 0.0) && (cdytail == 0.0) &&
		(adheighttail == 0.0) &&
		(bdheighttail == 0.0) && (cdheighttail == 0.0))
	{
		return det;
	}
...

{
	...
	det = adheight * (bdxcdy - cdxbdy)
		+ bdheight * (cdxady - adxcdy) + cdheight * (adxbdy - bdxady);
...
#ifdef ANSI_DECLARATORS
REAL
nonregular(struct mesh * m, struct behavior * b,
		   vertex pa, vertex pb, vertex pc, vertex pd)
#else /* not ANSI_DECLARATORS */
REAL
nonregular(m, b, pa, pb, pc, pd)
	 struct mesh *m;
	 struct behavior *b;
	 vertex pa;
	 vertex pb;
...
#ifdef ANSI_DECLARATORS
void
findcircumcenter(struct mesh *m, struct behavior *b,
				 vertex torg, vertex tdest, vertex tapex,
				 vertex circumcenter, REAL * xi, REAL * eta, REAL * minedge)
#else /* not ANSI_DECLARATORS */
void
findcircumcenter(m, b, torg, tdest, tapex, circumcenter, xi, eta, minedge)
	 struct mesh *m;
	 struct behavior *b;
	 vertex torg;
	 vertex tdest;
...

{
	...
	m->vertices.itembytes = m->triangles.itembytes = m->subsegs.itembytes =
		m->viri.itembytes = m->badsubsegs.itembytes =
		m->badtriangles.itembytes = m->flipstackers.itembytes =
		m->splaynodes.itembytes = 0;
...
#ifdef ANSI_DECLARATORS
void
checkmesh(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
void
checkmesh(m, b)
	 struct mesh *m;
...
#ifdef ANSI_DECLARATORS
void
checkdelaunay(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
void
checkdelaunay(m, b)
	 struct mesh *m;
...

{
	...
	while (triangleloop.tri != (triangle *) NULL)
	{
			 ...
			 triangleloop.orient++)
		{
			...
			if (shouldbedelaunay)
			{
				if (nonregular(m, b, triorg, tridest, triapex, oppoapex) >
					0.0)
				{
					if (!b->weighted)
					{
...
#ifdef ANSI_DECLARATORS
void
enqueuebadtriang(struct mesh *m, struct behavior *b, struct badtriang *badtri)
#else /* not ANSI_DECLARATORS */
void
...
#ifdef ANSI_DECLARATORS
void
enqueuebadtri(struct mesh *m, struct behavior *b, struct otri *enqtri,
			  REAL angle, vertex enqapex, vertex enqorg, vertex enqdest)
#else /* not ANSI_DECLARATORS */
void
enqueuebadtri(m, b, enqtri, angle, enqapex, enqorg, enqdest)
	 struct mesh *m;
	 struct behavior *b;
	 struct otri *enqtri;
	 REAL angle;
...
#ifdef ANSI_DECLARATORS
struct badtriang *
dequeuebadtriang(struct mesh *m)
#else /* not ANSI_DECLARATORS */
struct badtriang *
dequeuebadtriang(m)
	 struct mesh *m;
...
int
under60degrees(struct osub *sub1, struct osub *sub2)
{
...
int
clockwiseseg(struct mesh *m, struct osub *thissub, struct osub *nextsub)
{
...
int
counterclockwiseseg(struct mesh *m, struct osub *thissub,
					struct osub *nextsub)
{
	struct otri neighbortri;
	struct osub subsym;
...
int
splitpermitted(struct mesh *m, struct osub *testsubseg, REAL iradius)
{
...
#ifdef ANSI_DECLARATORS
int
checkseg4encroach(struct mesh *m, struct behavior *b,
				  struct osub *testsubseg, REAL iradius)
#else /* not ANSI_DECLARATORS */
int
checkseg4encroach(m, b, testsubseg, iradius)
	 struct mesh *m;
	 struct behavior *b;
...
#ifdef ANSI_DECLARATORS
void
testtriangle(struct mesh *m, struct behavior *b, struct otri *testtri)
#else /* not ANSI_DECLARATORS */
void
...
#ifdef ANSI_DECLARATORS
void
makevertexmap(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
void
makevertexmap(m, b)
	 struct mesh *m;
...
#ifdef ANSI_DECLARATORS
enum locateresult
preciselocate(struct mesh *m, struct behavior *b,
			  vertex searchpoint, struct otri *searchtri,
			  int stopatsubsegment)
#else /* not ANSI_DECLARATORS */
enum locateresult
preciselocate(m, b, searchpoint, searchtri, stopatsubsegment)
	 struct mesh *m;
	 struct behavior *b;
	 vertex searchpoint;
...
#ifdef ANSI_DECLARATORS
enum locateresult
locate(struct mesh *m, struct behavior *b,
	   vertex searchpoint, struct otri *searchtri)
#else /* not ANSI_DECLARATORS */
enum locateresult
...
#ifdef ANSI_DECLARATORS
void
insertsubseg(struct mesh *m, struct behavior *b, struct otri *tri,
			 int subsegmark)
#else /* not ANSI_DECLARATORS */
void
insertsubseg(m, b, tri, subsegmark)
	 struct mesh *m;
	 struct behavior *b;
...
#ifdef ANSI_DECLARATORS
void
flip(struct mesh *m, struct behavior *b, struct otri *flipedge)
#else /* not ANSI_DECLARATORS */
void
flip(m, b, flipedge)
	 struct mesh *m;
...
#ifdef ANSI_DECLARATORS
void
unflip(struct mesh *m, struct behavior *b, struct otri *flipedge)
#else /* not ANSI_DECLARATORS */
void
unflip(m, b, flipedge)
	 struct mesh *m;
...
#ifdef ANSI_DECLARATORS
enum insertvertexresult
insertvertex(struct mesh *m, struct behavior *b,
			 vertex newvertex, struct otri *searchtri,
			 struct osub *splitseg,
			 int segmentflaws, int triflaws, REAL iradius)
#else /* not ANSI_DECLARATORS */
enum insertvertexresult
insertvertex(m, b, newvertex, searchtri, splitseg,
...

{
	...
	if ((intersect == ONEDGE) || (intersect == OUTSIDE))
	{
		...
		if (mirrorflag)
		{
			if (counterclockwise(m, b, leftvertex, rightvertex, topvertex) <
				0.0)
			{
				fprintf(stderr, "Internal error in insertvertex():\n");
				fprintf(stderr,
			...
			if (counterclockwise(m, b, rightvertex, topvertex, newvertex) <
				0.0)
			{
				fprintf(stderr, "Internal error in insertvertex():\n");
				fprintf(stderr,
			...
			if (counterclockwise(m, b, topvertex, leftvertex, newvertex) <
				0.0)
			{
				fprintf(stderr, "Internal error in insertvertex():\n");
				fprintf(stderr,
		...
		if (doflip)
		{
			...
			else
			{
					...
					|| (leftvertex == m->infvertex3))
				{
					...
					/*   be applied.                                                */
					doflip =
						counterclockwise(m, b, newvertex, rightvertex,
										 farvertex) > 0.0;
				}
						 ...
						 (rightvertex == m->infvertex3))
				{
					...
					/*   be applied.                                                */
					doflip =
						counterclockwise(m, b, farvertex, leftvertex,
										 newvertex) > 0.0;
				}
				...
				if (doflip)
				{
					...
					{
						newflip =
							(struct flipstacker *) poolalloc(&m->
															 flipstackers);
					...
					{
						if (counterclockwise
							(m, b, leftvertex, newvertex, rightvertex) < 0.0)
						{
							fprintf(stderr,
...
#ifdef ANSI_DECLARATORS
void
triangulatepolygon(struct mesh *m, struct behavior *b,
				   struct otri *firstedge, struct otri *lastedge,
				   int edgecount, int doflip, int triflaws)
#else /* not ANSI_DECLARATORS */
void
triangulatepolygon(m, b, firstedge, lastedge, edgecount, doflip, triflaws)
	 struct mesh *m;
	 struct behavior *b;
...

{
	...
	if (bestnumber < edgecount - 2)
	{
		...
		triangulatepolygon(m, b, &besttri, lastedge, edgecount - bestnumber,
						   1, triflaws);
...
#ifdef ANSI_DECLARATORS
void
undovertex(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
void
undovertex(m, b)
	 struct mesh *m;
...
#else /* not ANSI_DECLARATORS */
void
vertexsort(sortarray, arraysize)
	 vertex *sortarray;
	 int arraysize;
#endif /* not ANSI_DECLARATORS */
...
#else /* not ANSI_DECLARATORS */
void
vertexmedian(sortarray, arraysize, median, axis)
	 vertex *sortarray;
	 int arraysize;
	 int median;
	 int axis;
...
#else /* not ANSI_DECLARATORS */
void
alternateaxes(sortarray, arraysize, axis)
	 vertex *sortarray;
	 int arraysize;
	 int axis;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void
mergehulls(struct mesh *m, struct behavior *b, struct otri *farleft,
...

{
	...
	while (1)
	{
		...
		if (leftfinished || (!rightfinished &&
							 (incircle
							  (m, b, upperleft, lowerleft, lowerright,
							   upperright) > 0.0)))
		{
...
#ifdef ANSI_DECLARATORS
void
divconqrecurse(struct mesh *m, struct behavior *b, vertex * sortarray,
			   int vertices, int axis,
			   struct otri *farleft, struct otri *farright)
#else /* not ANSI_DECLARATORS */
void
divconqrecurse(m, b, sortarray, vertices, axis, farleft, farright)
	 struct mesh *m;
	 struct behavior *b;
	 vertex *sortarray;
	 int vertices;
...

{
	...
	else if (vertices == 3)
	{
		...
		area =
			counterclockwise(m, b, sortarray[0], sortarray[1], sortarray[2]);
		if (area == 0.0)
		{
...
#ifdef ANSI_DECLARATORS
long
removeghosts(struct mesh *m, struct behavior *b, struct otri *startghost)
#else /* not ANSI_DECLARATORS */
long
removeghosts(m, b, startghost)
	 struct mesh *m;
	 struct behavior *b;
...
#ifdef ANSI_DECLARATORS
long
divconqdelaunay(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
long
divconqdelaunay(m, b)
	 struct mesh *m;
...
#ifdef ANSI_DECLARATORS
void
boundingbox(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
void
boundingbox(m, b)
	 struct mesh *m;
...
#ifdef ANSI_DECLARATORS
long
removebox(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
long
removebox(m, b)
	 struct mesh *m;
...
#ifdef ANSI_DECLARATORS
long
incrementaldelaunay(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
long
incrementaldelaunay(m, b)
	 struct mesh *m;
	 struct behavior *b;
#endif /* not ANSI_DECLARATORS */
...
#else /* not ANSI_DECLARATORS */
void
eventheapinsert(heap, heapsize, newevent)
	 struct event **heap;
	 int heapsize;
...
#else /* not ANSI_DECLARATORS */
void
eventheapify(heap, heapsize, eventnum)
	 struct event **heap;
	 int heapsize;
	 int eventnum;
#endif /* not ANSI_DECLARATORS */
...
#else /* not ANSI_DECLARATORS */
void
eventheapdelete(heap, heapsize, eventnum)
	 struct event **heap;
	 int heapsize;
	 int eventnum;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void
createeventheap(struct mesh *m, struct event ***eventheap,
...

{
	...
	*eventheap =
		(struct event **) trimalloc(maxevents * sizeof(struct event *));
...
#ifdef ANSI_DECLARATORS
int
rightofhyperbola(struct mesh *m, struct otri *fronttri, vertex newsite)
#else /* not ANSI_DECLARATORS */
int
rightofhyperbola(m, fronttri, newsite)
	 struct mesh *m;
...
#ifdef ANSI_DECLARATORS
REAL
circletop(struct mesh * m, vertex pa, vertex pb, vertex pc, REAL ccwabc)
#else /* not ANSI_DECLARATORS */
REAL
circletop(m, pa, pb, pc, ccwabc)
	 struct mesh *m;
	 vertex pa;
	 vertex pb;
	 vertex pc;
...

{
	...
	return pc[1] + (xac * bclen2 - xbc * aclen2 +
					sqrt(aclen2 * bclen2 * ablen2)) / (2.0 * ccwabc);
}
...
#else /* not ANSI_DECLARATORS */
void
check4deadevent(checktri, freeevents, eventheap, heapsize)
...
#ifdef ANSI_DECLARATORS
struct splaynode *
splay(struct mesh *m, struct splaynode *splaytree,
	  vertex searchpoint, struct otri *searchtri)
#else /* not ANSI_DECLARATORS */
struct splaynode *
splay(m, splaytree, searchpoint, searchtri)
	 struct mesh *m;
...
#ifdef ANSI_DECLARATORS
struct splaynode *
splayinsert(struct mesh *m, struct splaynode *splayroot,
			struct otri *newkey, vertex searchpoint)
#else /* not ANSI_DECLARATORS */
struct splaynode *
splayinsert(m, splayroot, newkey, searchpoint)
	 struct mesh *m;
...
#ifdef ANSI_DECLARATORS
struct splaynode *
circletopinsert(struct mesh *m, struct behavior *b,
				struct splaynode *splayroot,
				struct otri *newkey,
				vertex pa, vertex pb, vertex pc, REAL topy)
#else /* not ANSI_DECLARATORS */
struct splaynode *
circletopinsert(m, b, splayroot, newkey, pa, pb, pc, topy)
	 struct mesh *m;
	 struct behavior *b;
	 struct splaynode *splayroot;
	 struct otri *newkey;
...
#else /* not ANSI_DECLARATORS */
struct splaynode *
frontlocate(m, splayroot, bottommost, searchvertex, searchtri, farright)
	 struct mesh *m;
	 struct splaynode *splayroot;
...
#ifdef ANSI_DECLARATORS
long
sweeplinedelaunay(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
long
sweeplinedelaunay(m, b)
	 struct mesh *m;
	 struct behavior *b;
#endif /* not ANSI_DECLARATORS */
...
*/
				...
				check4deadevent(&searchtri, &freeevents, eventheap,
								&heapsize);
				...
				{
					splayroot =
						splayinsert(m, splayroot, &lefttri, nextvertex);
				}
		...
		if (check4events)
		{
			...
			if (lefttest > 0.0)
			{
				...
				newevent->ykey =
					circletop(m, leftvertex, midvertex, rightvertex,
							  lefttest);
...
#ifdef ANSI_DECLARATORS
long
delaunay(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
long
delaunay(m, b)
	 struct mesh *m;
...
#ifdef ANSI_DECLARATORS
int
reconstruct(struct mesh *m, struct behavior *b, int *trianglelist,
...
#else /* not ANSI_DECLARATORS */
int
reconstruct(m, b, trianglelist, triangleattriblist, trianglearealist,
...
#ifdef ANSI_DECLARATORS
long
reconstruct(struct mesh *m, struct behavior *b, char *elefilename,
			char *areafilename, char *polyfilename, FILE * polyfile)
#else /* not ANSI_DECLARATORS */
long
reconstruct(m, b, elefilename, areafilename, polyfilename, polyfile)
	 struct mesh *m;
	 struct behavior *b;
	 char *elefilename;
	 char *areafilename;
...

{
	...
	vertexarray =
		(triangle *) trimalloc(m->vertices.items * sizeof(triangle));
...
#ifdef ANSI_DECLARATORS
enum finddirectionresult
finddirection(struct mesh *m, struct behavior *b,
			  struct otri *searchtri, vertex searchpoint)
#else /* not ANSI_DECLARATORS */
enum finddirectionresult
finddirection(m, b, searchtri, searchpoint)
	 struct mesh *m;
	 struct behavior *b;
	 struct otri *searchtri;
	 vertex searchpoint;
...

{
	...
	while (leftflag)
	{
		...
		if (searchtri->tri == m->dummytri)
		{
			...
			fprintf(stderr, "  triangle leading from (%.12g, %.12g) to",
		...
		rightccw = leftccw;
		leftccw =
			counterclockwise(m, b, searchpoint, startvertex, leftvertex);
		leftflag = leftccw > 0.0;
	}
	while (rightflag)
	{
...
#ifdef ANSI_DECLARATORS
void
segmentintersection(struct mesh *m, struct behavior *b,
					struct otri *splittri, struct osub *splitsubseg,
					vertex endpoint2)
#else /* not ANSI_DECLARATORS */
void
segmentintersection(m, b, splittri, splitsubseg, endpoint2)
	 struct mesh *m;
	 struct behavior *b;
...
#ifdef ANSI_DECLARATORS
int
scoutsegment(struct mesh *m, struct behavior *b, struct otri *searchtri,
			 vertex endpoint2, int newmark)
#else /* not ANSI_DECLARATORS */
int
scoutsegment(m, b, searchtri, endpoint2, newmark)
	 struct mesh *m;
	 struct behavior *b;
	 struct otri *searchtri;
	 vertex endpoint2;
...
#ifdef ANSI_DECLARATORS
void
conformingedge(struct mesh *m, struct behavior *b,
...

{
	...
	else
	{
		if (success == VIOLATINGVERTEX)
		{
			...
			success =
				insertvertex(m, b, newvertex, &searchtri1, &brokensubseg, 0,
							 0, 0.0);
			if (success != SUCCESSFULVERTEX)
			{
...
#ifdef ANSI_DECLARATORS
void
delaunayfixup(struct mesh *m, struct behavior *b,
...
#ifdef ANSI_DECLARATORS
void
constrainededge(struct mesh *m, struct behavior *b,
				struct otri *starttri, vertex endpoint2, int newmark)
#else /* not ANSI_DECLARATORS */
void
constrainededge(m, b, starttri, endpoint2, newmark)
	 struct mesh *m;
	 struct behavior *b;
	 struct otri *starttri;
	 vertex endpoint2;
...
#ifdef ANSI_DECLARATORS
void
insertsegment(struct mesh *m, struct behavior *b,
			  vertex endpoint1, vertex endpoint2, int newmark)
#else /* not ANSI_DECLARATORS */
void
insertsegment(m, b, endpoint1, endpoint2, newmark)
	 struct mesh *m;
	 struct behavior *b;
	 vertex endpoint1;
	 vertex endpoint2;
...
#ifdef ANSI_DECLARATORS
void
markhull(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
void
markhull(m, b)
	 struct mesh *m;
...
#ifdef ANSI_DECLARATORS
void
formskeleton(struct mesh *m, struct behavior *b, int *segmentlist,
			 int *segmentmarkerlist, int numberofsegments)
#else /* not ANSI_DECLARATORS */
void
formskeleton(m, b, segmentlist, segmentmarkerlist, numberofsegments)
	 struct mesh *m;
	 struct behavior *b;
	 int *segmentlist;
	 int *segmentmarkerlist;
...
#ifdef ANSI_DECLARATORS
void
formskeleton(struct mesh *m, struct behavior *b,
			 FILE * polyfile, char *polyfilename)
#else /* not ANSI_DECLARATORS */
void
formskeleton(m, b, polyfile, polyfilename)
	 struct mesh *m;
	 struct behavior *b;
	 FILE *polyfile;
	 char *polyfilename;
...
#ifdef ANSI_DECLARATORS
void
infecthull(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
void
infecthull(m, b)
	 struct mesh *m;
...
#ifdef ANSI_DECLARATORS
void
plague(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
void
plague(m, b)
	 struct mesh *m;
...
#ifdef ANSI_DECLARATORS
void
regionplague(struct mesh *m, struct behavior *b, REAL attribute, REAL area)
#else /* not ANSI_DECLARATORS */
void
...
#ifdef ANSI_DECLARATORS
void
carveholes(struct mesh *m, struct behavior *b, REAL * holelist, int holes,
		   REAL * regionlist, int regions)
#else /* not ANSI_DECLARATORS */
void
carveholes(m, b, holelist, holes, regionlist, regions)
	 struct mesh *m;
	 struct behavior *b;
	 REAL *holelist;
	 int holes;
...
#ifdef ANSI_DECLARATORS
void
tallyencs(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
void
tallyencs(m, b)
	 struct mesh *m;
...
#ifdef ANSI_DECLARATORS
void
splitencsegs(struct mesh *m, struct behavior *b, int triflaws)
#else /* not ANSI_DECLARATORS */
void
splitencsegs(m, b, triflaws)
	 struct mesh *m;
	 struct behavior *b;
	 int triflaws;
#endif /* not ANSI_DECLARATORS */
			   ...
			   && (m->steinerleft != 0))
		{
				...
				&& (edest == encloop->subsegdest))
			{
				...
				if (!b->noexact)
				{
					...
					multiplier =
						counterclockwise(m, b, eorg, edest, newvertex);
					divisor =
...
#ifdef ANSI_DECLARATORS
void
tallyfaces(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
void
tallyfaces(m, b)
	 struct mesh *m;
...
#ifdef ANSI_DECLARATORS
void
splittriangle(struct mesh *m, struct behavior *b, struct badtriang *badtri)
#else /* not ANSI_DECLARATORS */
void
...

{
		...
		(bdest == badtri->triangdest) && (bapex == badtri->triangapex))
	{
		...
		else
		{
			...
			success =
				insertvertex(m, b, newvertex, &badotri, (struct osub *) NULL,
							 1, 1, minedge);
			if (success == SUCCESSFULVERTEX)
			{
				if (m->steinerleft > 0)
...
#ifdef ANSI_DECLARATORS
void
enforcequality(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
void
enforcequality(m, b)
	 struct mesh *m;
...

{
	...
	if (!b->quiet && (m->badsubsegs.items > 0) && (m->steinerleft == 0))
	{
		...
		{
			fprintf(stderr,
					"  an encroached subsegment, and therefore might not be truly\n");
		}
		else
		{
			fprintf(stderr,
...
#ifdef ANSI_DECLARATORS
void
highorder(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
void
highorder(m, b)
	 struct mesh *m;
			 ...
			 triangleloop.orient++)
		{
				...
				|| (trisym.tri == m->dummytri))
			{
				...
				setvertextype(newvertex,
							  trisym.tri ==
							  m->dummytri ? FREEVERTEX : SEGMENTVERTEX);
				if (b->usesegments)
				{
...
#ifdef ANSI_DECLARATORS
char *
readline(char *string, FILE * infile, char *infilename)
#else /* not ANSI_DECLARATORS */
char *
readline(string, infile, infilename)
	 char *string;
	 FILE *infile;
	 char *infilename;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void
readnodes(struct mesh *m, struct behavior *b, char *nodefilename,
		  char *polyfilename, FILE ** polyfile)
#else /* not ANSI_DECLARATORS */
void
readnodes(m, b, nodefilename, polyfilename, polyfile)
	 struct mesh *m;
	 struct behavior *b;
	 char *nodefilename;
	 char *polyfilename;
...
#ifdef ANSI_DECLARATORS
void
transfernodes(struct mesh *m, struct behavior *b, REAL * pointlist,
			  REAL * pointattriblist, int *pointmarkerlist,
			  int numberofpoints, int numberofpointattribs)
#else /* not ANSI_DECLARATORS */
void
transfernodes(m, b, pointlist, pointattriblist, pointmarkerlist,
...
#ifdef ANSI_DECLARATORS
void
readholes(struct mesh *m, struct behavior *b,
		  FILE * polyfile, char *polyfilename, REAL ** hlist, int *holes,
		  REAL ** rlist, int *regions)
#else /* not ANSI_DECLARATORS */
void
readholes(m, b, polyfile, polyfilename, hlist, holes, rlist, regions)
	 struct mesh *m;
	 struct behavior *b;
	 FILE *polyfile;
	 char *polyfilename;
...
#ifdef ANSI_DECLARATORS
void
finishfile(FILE * outfile, int argc, char **argv)
#else /* not ANSI_DECLARATORS */
void
...
#ifdef ANSI_DECLARATORS
void
writenodes(struct mesh *m, struct behavior *b, REAL ** pointlist,
		   REAL ** pointattriblist, int **pointmarkerlist)
#else /* not ANSI_DECLARATORS */
void
writenodes(m, b, pointlist, pointattriblist, pointmarkerlist)
	 struct mesh *m;
	 struct behavior *b;
	 REAL **pointlist;
	 REAL **pointattriblist;
...
#ifdef ANSI_DECLARATORS
void
writenodes(struct mesh *m, struct behavior *b, char *nodefilename,
		   int argc, char **argv)
#else /* not ANSI_DECLARATORS */
void
writenodes(m, b, nodefilename, argc, argv)
	 struct mesh *m;
	 struct behavior *b;
	 char *nodefilename;
	 int argc;
...

{
	...
	while (vertexloop != (vertex) NULL)
	{
		if (!b->jettison || (vertextype(vertexloop) != UNDEADVERTEX))
		{
			...
			fprintf(outfile, "%4d    %.17g  %.17g", vertexnumber,
					vertexloop[0], vertexloop[1]);
			for (i = 0; i < m->nextras; i++)
			{
				/* Write an attribute. */
...
#ifdef ANSI_DECLARATORS
void
numbernodes(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
void
numbernodes(m, b)
	 struct mesh *m;
...
#ifdef ANSI_DECLARATORS
void
writeelements(struct mesh *m, struct behavior *b,
			  int **trianglelist, REAL ** triangleattriblist)
#else /* not ANSI_DECLARATORS */
void
writeelements(m, b, trianglelist, triangleattriblist)
	 struct mesh *m;
	 struct behavior *b;
	 int **trianglelist;
	 REAL **triangleattriblist;
...
#ifdef ANSI_DECLARATORS
void
writeelements(struct mesh *m, struct behavior *b, char *elefilename,
			  int argc, char **argv)
#else /* not ANSI_DECLARATORS */
void
writeelements(m, b, elefilename, argc, argv)
	 struct mesh *m;
	 struct behavior *b;
	 char *elefilename;
	 int argc;
...

{
	...
	{
		*triangleattriblist =
			(REAL *) trimalloc(m->triangles.items * m->eextras *
							   sizeof(REAL));
	}
	tlist = *trianglelist;
...
#ifdef ANSI_DECLARATORS
void
writepoly(struct mesh *m, struct behavior *b,
		  int **segmentlist, int **segmentmarkerlist)
#else /* not ANSI_DECLARATORS */
void
writepoly(m, b, segmentlist, segmentmarkerlist)
	 struct mesh *m;
	 struct behavior *b;
	 int **segmentlist;
	 int **segmentmarkerlist;
...
#ifdef ANSI_DECLARATORS
void
writepoly(struct mesh *m, struct behavior *b, char *polyfilename,
		  REAL * holelist, int holes, REAL * regionlist, int regions,
		  int argc, char **argv)
#else /* not ANSI_DECLARATORS */
void
writepoly(m, b, polyfilename, holelist, holes, regionlist, regions,
		  argc, argv)
	 struct mesh *m;
	 struct behavior *b;
	 char *polyfilename;
...

{
	...
	{
		*segmentmarkerlist =
			(int *) trimalloc(m->subsegs.items * sizeof(int));
	}
	slist = *segmentlist;
...
#ifdef ANSI_DECLARATORS
void
writeedges(struct mesh *m, struct behavior *b,
		   int **edgelist, int **edgemarkerlist)
#else /* not ANSI_DECLARATORS */
void
writeedges(m, b, edgelist, edgemarkerlist)
	 struct mesh *m;
	 struct behavior *b;
	 int **edgelist;
	 int **edgemarkerlist;
...
#ifdef ANSI_DECLARATORS
void
writeedges(struct mesh *m, struct behavior *b, char *edgefilename,
		   int argc, char **argv)
#else /* not ANSI_DECLARATORS */
void
writeedges(m, b, edgefilename, argc, argv)
	 struct mesh *m;
	 struct behavior *b;
	 char *edgefilename;
	 int argc;
...
#ifdef ANSI_DECLARATORS
void
writevoronoi(struct mesh *m, struct behavior *b, REAL ** vpointlist,
			 REAL ** vpointattriblist, int **vpointmarkerlist,
			 int **vedgelist, int **vedgemarkerlist, REAL ** vnormlist)
#else /* not ANSI_DECLARATORS */
void
writevoronoi(m, b, vpointlist, vpointattriblist, vpointmarkerlist,
...
#ifdef ANSI_DECLARATORS
void
writevoronoi(struct mesh *m, struct behavior *b, char *vnodefilename,
			 char *vedgefilename, int argc, char **argv)
#else /* not ANSI_DECLARATORS */
void
writevoronoi(m, b, vnodefilename, vedgefilename, argc, argv)
	 struct mesh *m;
	 struct behavior *b;
	 char *vnodefilename;
	 char *vedgefilename;
...

{
	...
	{
		*vpointlist =
			(REAL *) trimalloc(m->triangles.items * 2 * sizeof(REAL));
	}
	/* Allocate memory for Voronoi vertex attributes if necessary. */
	if (*vpointattriblist == (REAL *) NULL)
	{
		*vpointattriblist =
			(REAL *) trimalloc(m->triangles.items * m->nextras *
							   sizeof(REAL));
	}
	*vpointmarkerlist = (int *) NULL;
	...
	while (triangleloop.tri != (triangle *) NULL)
	{
		...
		dest(triangleloop, tdest);
		apex(triangleloop, tapex);
		findcircumcenter(m, b, torg, tdest, tapex, circumcenter, &xi, &eta,
						 &dum);
#ifdef TRILIBRARY
		/* X and y coordinates. */
...
#ifdef ANSI_DECLARATORS
void
writeneighbors(struct mesh *m, struct behavior *b, int **neighborlist)
#else /* not ANSI_DECLARATORS */
void
writeneighbors(m, b, neighborlist)
	 struct mesh *m;
	 struct behavior *b;
	 int **neighborlist;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void
writeneighbors(struct mesh *m, struct behavior *b, char *neighborfilename,
			   int argc, char **argv)
#else /* not ANSI_DECLARATORS */
void
writeneighbors(m, b, neighborfilename, argc, argv)
	 struct mesh *m;
	 struct behavior *b;
...

{
	...
	{
		*neighborlist =
			(int *) trimalloc(m->triangles.items * 3 * sizeof(int));
	}
	nlist = *neighborlist;
	index = 0;
#else /* not TRILIBRARY */
...
#ifdef ANSI_DECLARATORS
void
writeoff(struct mesh *m, struct behavior *b, char *offfilename,
		 int argc, char **argv)
#else /* not ANSI_DECLARATORS */
void
writeoff(m, b, offfilename, argc, argv)
	 struct mesh *m;
	 struct behavior *b;
	 char *offfilename;
	 int argc;
...
#ifdef ANSI_DECLARATORS
void
quality_statistics(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
void
...
#ifdef ANSI_DECLARATORS
void
statistics(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
void
statistics(m, b)
	 struct mesh *m;
...

{
	...
	if (!b.quiet)
	{
		...
		fprintf(stderr, " milliseconds:  %ld\n",
				1000l * (tv2.tv_sec - tv1.tv_sec) + (tv2.tv_usec -
													 tv1.tv_usec) / 1000l);
	}
#endif /* not NO_TIMER */
	...
	if (!b.quiet)
	{
		...
		if (b.poly && !b.refine)
		{
			fprintf(stderr, "Hole milliseconds:  %ld\n",
					1000l * (tv4.tv_sec - tv3.tv_sec) + (tv4.tv_usec -
														 tv3.tv_usec) /
					1000l);
		}
	}

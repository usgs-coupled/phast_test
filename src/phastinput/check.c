...
/* ---------------------------------------------------------------------- */
{
		...
		&& (print_xyz_wells.count_properties > 0))
	{
		...
		sprintf(error_string,
				"The file %s only applies to simulations with chemistry.\n"
	...
	if ((save_final_heads == TRUE) && (head_ic_file_warning == TRUE))
	{
		...
		sprintf(error_string,
				"Writing heads to file (PRINT_FREQUENCY; -save_final_heads true)\n\twill overwrite a file used for initial conditions, %s.",
				name);
...
/* ---------------------------------------------------------------------- */
{
	...
	for (i = 0; i < 3; i++)
	{
		if (axes[i] == FALSE)
		{
			if (grid[i].count_coord != 2)
			{
				input_error++;
				sprintf(error_string,
						"Coordinate %c was not defined to be active in -chemistry_dimensions identifier of GRID keyword,\n\texactly 2 nodes must be defined in this direction with GRID.",
						grid[i].c);
				error_msg(error_string, CONTINUE);
			}
		}
	...
	if (j == 0)
	{
		input_error++;
		sprintf(error_string,
				"No coordinate directions were defined to be active in -chemistry_dimensions identifier of GRID keyword.");
		error_msg(error_string, CONTINUE);
	}
	return (OK);
}
 ...
 */
	...
	if (units.horizontal.defined == FALSE)
	{
		input_error++;
		sprintf(error_string,
				"Horizontal grid units not defined in UNITS data block.");
		error_msg(error_string, CONTINUE);
	}
	if (units.vertical.defined == FALSE)
	{
		input_error++;
		sprintf(error_string,
				"Vertical grid units not defined in UNITS data block.");
		error_msg(error_string, CONTINUE);
	}
	...
	if (units.k.defined == FALSE)
	{
		input_error++;
		sprintf(error_string,
				"Hydraulic conductivity units not defined in UNITS data block.");
		error_msg(error_string, CONTINUE);
	}
	else
	{
	...
	if (units.s.defined == FALSE)
	{
		input_error++;
		sprintf(error_string,
				"Specific storage units not defined in UNITS data block.");
		error_msg(error_string, CONTINUE);
	}
	if (units.alpha.defined == FALSE && flow_only == FALSE)
	{
		input_error++;
		sprintf(error_string,
				"Dispersivity units not defined in UNITS data block.");
		error_msg(error_string, CONTINUE);
	}
	if (count_leaky > 0 && units.leaky_k.defined == FALSE)
	{
		input_error++;
		sprintf(error_string,
				"Hydraulic conductivity units for leaky boundary not defined in UNITS data block.");
		error_msg(error_string, CONTINUE);
	}
	else
	{
		units.leaky_k.input_to_user =
			units.leaky_k.input_to_si * (1 / si_to_user);
	}
	if (count_leaky > 0 && units.leaky_thick.defined == FALSE)
	{
		input_error++;
		sprintf(error_string,
				"Units of thickness for leaky boundary not defined in UNITS data block.");
		error_msg(error_string, CONTINUE);
	}
	...
	if (count_wells > 0 && units.well_pumpage.defined == FALSE)
	{
		input_error++;
		sprintf(error_string,
				"Well pumpage units not defined in UNITS data block.");
		error_msg(error_string, CONTINUE);
	}
	else
	{
	...
	if (count_wells > 0 && units.well_diameter.defined == FALSE)
	{
		input_error++;
		sprintf(error_string,
				"Well diameter units not defined in UNITS data block.");
		error_msg(error_string, CONTINUE);
	}
	else
	{
	...
	if (count_rivers > 0 && units.river_bed_k.defined == FALSE)
	{
		input_error++;
		sprintf(error_string,
				"River bed hydraulic conductivity units not defined in UNITS data block.");
		error_msg(error_string, CONTINUE);
	}
	else
	{
		units.river_bed_k.input_to_user =
			units.river_bed_k.input_to_si * (1 / si_to_user);
	}
	if (count_rivers > 0 && units.river_bed_thickness.defined == FALSE)
	{
		input_error++;
		sprintf(error_string,
				"River bed thickness units not defined in UNITS data block.");
		error_msg(error_string, CONTINUE);
	}
	else
	{
		units.river_bed_thickness.input_to_user =
	...
	if (drains.size() > 0 && units.drain_bed_k.defined == FALSE)
	{
		input_error++;
		sprintf(error_string,
				"Drain bed hydraulic conductivity units not defined in UNITS data block.");
		error_msg(error_string, CONTINUE);
	}
	else
	{
		units.drain_bed_k.input_to_user =
			units.drain_bed_k.input_to_si * (1 / si_to_user);
	}
	if (drains.size() > 0 && units.drain_bed_thickness.defined == FALSE)
	{
		input_error++;
		sprintf(error_string,
				"Drain bed thickness units not defined in UNITS data block.");
		error_msg(error_string, CONTINUE);
	}
	else
	{
		units.drain_bed_thickness.input_to_user =
	...
	if (drains.size() > 0 && units.drain_width.defined == FALSE)
	{
		input_error++;
		sprintf(error_string,
				"Drain width units not defined in UNITS data block.");
		error_msg(error_string, CONTINUE);
	}
	else
	{
...
/* ---------------------------------------------------------------------- */
int
check_time_units(struct time *time_ptr, int required, const char *errstr)
/* ---------------------------------------------------------------------- */
{
/*
 ...
 */
	...
	{
		input_error++;
		sprintf(error_string, "Must enter time unit (T), not STEP, %s.",
				errstr);
		error_msg(error_string, CONTINUE);
		return (ERROR);
	}
 ...
 */
		if (units_conversion
			(time_ptr->input, "s", &(time_ptr->input_to_si), TRUE) == ERROR)
		{
			input_error++;
	 ...
	 */
	if (time_ptr->type != UNDEFINED && required == FALSE
		&& time_ptr->value > 0)
	{
		if (current_time_step.value * current_time_step.input_to_user >
			time_ptr->value * time_ptr->input_to_user)
		{
			input_error++;
			sprintf(error_string,
					"Print frequency, %g, for %s, must not be less than time step, %g %c.",
					time_ptr->value * time_ptr->input_to_user, errstr,
...
/* ---------------------------------------------------------------------- */
int
check_ss_time_units(struct time *time_ptr, const char *errstr)
/* ---------------------------------------------------------------------- */
{
/*
 *   function calcuates time_ptr->input_to_usr
...
/* ---------------------------------------------------------------------- */
{
	...
	for (i = 0; i < nxyz; i++)
	{
		...
		{
			input_error++;
			sprintf(error_string, "Head initial condition not defined %s",
					tag);
			error_msg(error_string, CONTINUE);
		}
		...
		{
			input_error++;
			sprintf(error_string, "Solution initial condition not defined %s",
					tag);
			error_msg(error_string, CONTINUE);
		}
		...
		if (cells[i].specified)
		{
			...
			{
				sprintf(error_string,
						"River is defined for specified-value cell; river will be ignored %s",
						tag);
				warning_msg(error_string);
			}
			...
			{
				sprintf(error_string,
						"Drain is defined for specified-value cell; drain will be ignored %s",
						tag);
				warning_msg(error_string);
			}
			...
			if (!rit->bc_head_defined)
			{
				...
				for (std::list < BC_info >::reverse_iterator rit1 =
					 cells[i].all_bc_info->rbegin();
					 rit1 != cells[i].all_bc_info->rend(); rit1++)
				{
				...
				if (!rit->bc_head_defined)
				{
					input_error++;
					sprintf(error_string,
							"Head for SPECIFIED_BC not defined %s", tag);
					error_msg(error_string, CONTINUE);
				}
			}
			...
			if (rit->bc_solution_type == UNDEFINED && flow_only == FALSE)
			{
				...
				if (rit->bc_solution_type == UNDEFINED)
				{
					input_error++;
					sprintf(error_string,
							"Solution type for SPECIFIED_BC not defined %s",
							tag);
					error_msg(error_string, CONTINUE);
				}
			}
			...
			if (!rit->bc_solution_defined && flow_only == FALSE)
			{
				...
				if (!rit->bc_solution_defined)
				{
					input_error++;
					sprintf(error_string,
							"Solution for SPECIFIED_BC not defined %s", tag);
					error_msg(error_string, CONTINUE);
				}
			}
		}
		...
		if (cells[i].flux)
		{
				 ...
				 rit != cells[i].all_bc_info->rend(); rit++)
			{
				...
				{
					input_error++;
					sprintf(error_string, "Flux for FLUX_BC not defined %s",
							tag);
					error_msg(error_string, CONTINUE);
				}
				...
				if (rit->bc_solution_type != ASSOCIATED && flow_only == FALSE)
				{
					input_error++;
					sprintf(error_string,
							"Associated solution for FLUX_BC not defined %s",
							tag);
					error_msg(error_string, CONTINUE);
					break;
				}
		...
		if (cells[i].leaky)
		{
				 ...
				 rit != cells[i].all_bc_info->rend(); rit++)
			{
				...
				{
					input_error++;
					sprintf(error_string, "Head for LEAKY_BC not defined %s",
							tag);
					error_msg(error_string, CONTINUE);
				}
				...
				if (!rit->bc_k_defined)
				{
					input_error++;
					sprintf(error_string,
							"Hydraulic conductivity for LEAKY_BC not defined %s",
							tag);
					error_msg(error_string, CONTINUE);
				}
				...
				if (!rit->bc_thick_defined)
				{
					input_error++;
					sprintf(error_string,
							"Thickness for LEAKY_BC not defined %s", tag);
					error_msg(error_string, CONTINUE);
				}
				...
				if (rit->bc_solution_type != ASSOCIATED && flow_only == FALSE)
				{
					input_error++;
					sprintf(error_string,
							"Associated solution for LEAKY_BC not defined %s",
							tag);
					error_msg(error_string, CONTINUE);
					break;
				}
 ...
 */
		...
		{
			input_error++;
			sprintf(error_string, "X hydraulic conductivity not defined %s",
					tag);
			error_msg(error_string, CONTINUE);
		}
/* 
 ...
 */
		...
		{
			input_error++;
			sprintf(error_string, "Y hydraulic conductivity not defined %s",
					tag);
			error_msg(error_string, CONTINUE);
		}
/* 
 ...
 */
		...
		{
			input_error++;
			sprintf(error_string, "Z hydraulic conductivity not defined %s",
					tag);
			error_msg(error_string, CONTINUE);
		}
/* 
 ...
 */
		...
		{
			input_error++;
			sprintf(error_string, "Longitudinal dispersivity not defined %s",
					tag);
			error_msg(error_string, CONTINUE);
		}
/* 
 ...
 */
		...
		{
			input_error++;
			sprintf(error_string, "Horizontal dispersivity not defined %s",
					tag);
			error_msg(error_string, CONTINUE);
		}
 ...
 */
		...
		{
			input_error++;
			sprintf(error_string, "Vertical dispersivity not defined %s",
					tag);
			error_msg(error_string, CONTINUE);
		}

		if (cells[i].alpha_horizontal_defined == TRUE
			&& cells[i].alpha_long_defined == TRUE)
		{
	...
	{
		sprintf(error_string,
				"Horizontal, vertical, and longitudinal dispersivities are not equal for at least one element.\n\tHowever, cross-dispersion calculation is disabled (-cross_dispersion in SOLUTION_METHOD).");
		warning_msg(error_string);
	}
	return (OK);
}
...
/* ---------------------------------------------------------------------- */
int
ijk_to_n_no_error(int i, int j, int k)
/* ---------------------------------------------------------------------- */
{
	int n, return_value;
	n = nx * ny * k + nx * j + i;
...
/* ---------------------------------------------------------------------- */
int
check_leaky_for_face(int i, int j)
/* ---------------------------------------------------------------------- */
{
/* 
 * i is node number 
 ...
 */
	...
	for (dz = -1; dz <= 0; dz++)
	{
		for (dy = -1; dy <= 0; dy++)
		{
			for (dx = -1; dx < 1; dx++)
			{
				n = ijk_to_n_no_error(cells[i].ix + dx, cells[i].iy + dy,
									  cells[i].iz + dz);
				if (n != -1 && cells[n].is_element
					&& cells[n].elt_active == TRUE)
				{
					e[k] = TRUE;
				}
				else
 ...
 */
	...
	if (j == 0)
	{
		if ((e[0] ^ e[1]) || (e[2] ^ e[3]) || (e[4] ^ e[5]) || (e[6] ^ e[7]))
		{
			return_value = OK;
		}
	}
	...
	{
		if ((e[0] ^ e[2]) || (e[1] ^ e[3]) || (e[4] ^ e[6]) || (e[5] ^ e[7]))
		{
			return_value = OK;
		}
	}
	...
	{
		if ((e[0] ^ e[4]) || (e[1] ^ e[5]) || (e[2] ^ e[6]) || (e[3] ^ e[7]))
		{
			return_value = OK;
		}
	}
...
/* ---------------------------------------------------------------------- */
{
	...
	if (steady_flow == FALSE)
	{
		if (((print_input_hdf_ss_vel == TRUE)
			...
			|| ((print_input_xyz_ss_vel == TRUE)
				&& (print_input_xyz_ss_vel_defined == TRUE))
			||
			((print_input_ss_vel == TRUE
			  && print_input_ss_vel_defined == TRUE)))
		{
			sprintf(error_string,
					"Identifiers Steady_flow_velocities, HDF_steady_flow_velocities,\n"
	...
	if (steady_flow == TRUE)
	{
		if (min_ss_time_step.value_defined == FALSE)
		{
			...
			{
				min_ss_time_step.input =
					string_duplicate(current_time_step.input);
			}
		}
		check_ss_time_units(&min_ss_time_step,
							"minimum time_step in STEADY_FLOW data block.");
		if (max_ss_time_step.value_defined == FALSE)
		{
			max_ss_time_step.value = current_time_step.value * 1000;
			if (current_time_step.input != NULL)
			{
				max_ss_time_step.input =
					string_duplicate(current_time_step.input);
			}
		}
		check_ss_time_units(&max_ss_time_step,
							"maximum time_step in STEADY_FLOW data block.");
	}
	if (time_start.value_defined == FALSE)
	{
		time_start.value = 0.0;
	}
	check_ss_time_units(&time_start,
						"starting time in TIME_CONTROL data block.");
	return (OK);
}

/* ---------------------------------------------------------------------- */
int
check_time_series_data(void)
/* ---------------------------------------------------------------------- */
{
/*
 *   Time parameters
 */
	...
	for (i = 0; i < count_bc; i++)
	{
		if (bc[i]->bc_head != NULL)
			check_time_series(bc[i]->bc_head, TRUE, "bc head");
		if (bc[i]->bc_flux != NULL)
			check_time_series(bc[i]->bc_flux, TRUE, "bc head");
		if (bc[i]->bc_solution != NULL && flow_only == FALSE)
			check_time_series(bc[i]->bc_solution, TRUE, "bc solution");
	}
	/*
	 * Rivers
	 */
	...
	if (time_step.properties != NULL)
		check_time_series(&time_step, TRUE, "time step");
	/*
	 *  Sort end times
	 */
	if (time_end == NULL)
	{
		input_error++;
		error_msg("Data not defined for end of simulation, TIME_CONTROL",
				  CONTINUE);
	}
	else if (time_end[0].value_defined == FALSE)
	{
		input_error++;
		error_msg
			("Incorrect data for end of simulation periods, TIME_CONTROL",
			 CONTINUE);
	}
	else
	{
		...
		qsort(time_end, (size_t) count_time_end, sizeof(struct time),
			  time_compare);
	}
	/*
	 *  Print frequencies
	 */
	if (print_velocity.properties != NULL)
		check_time_series(&print_velocity, FALSE, "print velocity");
	if (print_hdf_velocity.properties != NULL)
		check_time_series(&print_hdf_velocity, FALSE, "print hdf velocity");
	if (print_xyz_velocity.properties != NULL)
		check_time_series(&print_xyz_velocity, FALSE, "print xyz velocity");
	if (print_head.properties != NULL)
		check_time_series(&print_head, FALSE, "print head");
	if (print_hdf_head.properties != NULL)
		check_time_series(&print_hdf_head, FALSE, "print hdf head");
	if (print_xyz_head.properties != NULL)
		check_time_series(&print_xyz_head, FALSE, "print xyz head");
	if (flow_only == FALSE)
	{
		if (print_force_chem.properties != NULL)
			check_time_series(&print_force_chem, FALSE,
							  "force chemistry print");
		if (print_hdf_chem.properties != NULL)
			check_time_series(&print_hdf_chem, FALSE, "print hdf chemistry");
		if (print_xyz_chem.properties != NULL)
			check_time_series(&print_xyz_chem, FALSE, "print xyz chemistry");
		if (print_comp.properties != NULL)
			check_time_series(&print_comp, FALSE, "print components");
		if (print_xyz_comp.properties != NULL)
			check_time_series(&print_xyz_comp, FALSE, "print xyz components");
	}
	if (print_wells.properties != NULL)
		check_time_series(&print_wells, FALSE, "print wells");
	if (print_xyz_wells.properties != NULL)
		check_time_series(&print_xyz_wells, FALSE, "print xyz wells");
	if (print_statistics.properties != NULL)
		check_time_series(&print_statistics, FALSE, "print statistics");
	if (print_flow_balance.properties != NULL)
		check_time_series(&print_flow_balance, FALSE, "print flow balance");
	if (print_bc_flow.properties != NULL)
		check_time_series(&print_bc_flow, FALSE, "print bc flow");
	if (print_conductances.properties != NULL)
		check_time_series(&print_conductances, FALSE, "print conductances");
	if (print_bc.properties != NULL)
		check_time_series(&print_bc, FALSE, "print boundary conditions");
	if (print_end_of_period.properties != NULL)
		check_time_series(&print_end_of_period, FALSE,
						  "print end of period default");
	if (print_restart.properties != NULL)
	 ...
	 * check defaults
	 */
	check_time_units(&current_print_velocity, FALSE,
					 "current_print_velocity");
	check_time_units(&current_print_hdf_velocity, FALSE,
					 "current_print_hdf_velocity");
	check_time_units(&current_print_xyz_velocity, FALSE,
					 "current_print_xyz_velocity");
	check_time_units(&current_print_head, FALSE, "current_print_head");
	check_time_units(&current_print_hdf_head, FALSE,
					 "current_print_hdf_head");
	check_time_units(&current_print_xyz_head, FALSE,
					 "current_print_xyz_head");
	if (flow_only == FALSE)
	{
		check_time_units(&current_print_force_chem, FALSE,
						 "current_print_force_chem");
		check_time_units(&current_print_hdf_chem, FALSE,
						 "current_print_hdf_chem");
		check_time_units(&current_print_xyz_chem, FALSE,
						 "current_print_xyz_chem");
		check_time_units(&current_print_comp, FALSE, "current_print_comp");
		check_time_units(&current_print_xyz_comp, FALSE,
						 "current_print_xyz_comp");
	}
	check_time_units(&current_print_wells, FALSE, "current_print_wells");
	check_time_units(&current_print_xyz_wells, FALSE,
					 "current_print_xyz_wells");
	check_time_units(&current_print_statistics, FALSE,
					 "current_print_statistics");
	check_time_units(&current_print_flow_balance, FALSE,
					 "current_print_flow_balance");
	check_time_units(&current_print_bc_flow, FALSE, "current_print_bc_flow");
	check_time_units(&current_print_conductances, FALSE,
					 "current_print_conductances");
...
/* ---------------------------------------------------------------------- */
int
check_time_series(struct time_series *ts_ptr, int start_at_zero,
...
/* ---------------------------------------------------------------------- */
{
	...
	{
		if (check_time_units
			(&(ts_ptr->properties[i]->time), FALSE, "time series") == ERROR)
		{
			input_error++;
		...
		{
			if (check_time_units
				(&(ts_ptr->properties[i]->time_value), FALSE,
				 "time series") == ERROR)
			{
				input_error++;
				sprintf(error_string, "Time units error: %s", errorString);
	...
	if (start_at_zero == TRUE)
	{
		if (ts_ptr->properties != NULL)
		{
			if (ts_ptr->properties[0]->time.value != 0.0)
			{
				input_error++;
				sprintf(error_string,
						"Time series must start at time zero: %s",
						errorString);
				error_msg(error_string, CONTINUE);
				return (ERROR);
			}
		...
		{
			input_error++;
			sprintf(error_string, "Time series is unreadable: %s",
					errorString);
			error_msg(error_string, CONTINUE);
			return (ERROR);
		}

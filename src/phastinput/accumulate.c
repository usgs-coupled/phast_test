	...
	for (i = 0; i < count_bc; i++)
	{
		...
		else
		{
			...
			case BC_info::BC_FLUX:
				...
				distribute_flux_bc(i, list_of_cells, face_areas, tag);
			...
			case BC_info::BC_LEAKY:
				...
				distribute_leaky_bc(i, list_of_cells, face_areas, tag);
				break;
			}
		}
	}
...
/* ---------------------------------------------------------------------- */
{
	...
	*i2 = count_coord - 1;
	...
	if (*i2 == 0)
	{
		warning_msg("River or drain polygon is outside model domain.");
		return (ERROR);
	}
	return (OK);
}
							   ...
							   Cell_Face face)
{
	...
	for (f = (int) CF_X; f <= (int) CF_Z; f++)
	{
		...
		if (cf == face || face == CF_ALL)
		{
			...
			faces_intersect_polyhedron(i, temp_list);
		}
...
faces_intersect_polyhedron(int i, std::list < int >&list_of_numbers)
{
	...
	// free gpc_polygons
	std::map<int, gpc_polygon *>::iterator it;
	for (it = face_areas.begin(); it != face_areas.end(); it++)
	{
		if (it->second != NULL)
		{
			gpc_free_polygon(it->second);
			free_check_null(it->second);
		}
	}
}
#ifdef SKIP

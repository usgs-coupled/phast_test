...
export PATCH=`echo $VER | sed -e 's/[^\.]*\.//' -e 's/[^\.]*\.//'`
...
# build phast.msi x64
  MsBuild.exe "${MSI_SLN}" /t:Clean /p:Configuration=Release /p:Platform=x64 /p:TargetName=${FULLPKG}-x64 /p:Major=${MAJOR} /p:Minor=${MINOR} /p:Patch=${PATCH} /p:Build=${REL}  && \
# build phast.msi x64

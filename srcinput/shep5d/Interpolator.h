...
class Interpolator {
  protected:
   ...
   LNEXT[NMAX+1];               //Array of length N containing next-node
                                //  indices.  See STOREM.
double      *WS,                 //Work space array of length >= NT**2 for
                                //  NT=(M+1)(M+2)/2.
  ...
  public:
	Interpolator (void);
       ...
       ~Interpolator ();
        ...
        double **QSMTST3(int*, double[][3], char);
        void QSHEPM(int&);
        double QSMVAL(double*);
        void QSMGRD();
        ...
        void STOREM(int&);
        void get_node_coords(double*,int);
        double **getX(){ return X;};     // returns array of coordinates
        double *getW(){ return W;};      // returns array of weights
        int getN(){ return N; };        // returns number of known nodes
        void TFUN3(int,int,double*,int,int,double*,double,double,double);

...
/* ---------------------------------------------------------------------- */
{
	if (time_series_realloc(time_series_ptr) == ERROR) return(ERROR);
	time_series_ptr->properties[time_series_ptr->count_properties - 1] = property_time_ptr;
	return(OK);
}
/* ---------------------------------------------------------------------- */
 ...
 */
	if (time_series_ptr->count_properties > 1) {
		qsort(time_series_ptr->properties, 
		      ...
		      (size_t) sizeof(struct property_time *),
		      property_time_compare);
	}
	return(OK);
}
/* ---------------------------------------------------------------------- */
...
/* ---------------------------------------------------------------------- */
{
	...
	property_time_ptr = (struct property_time *) malloc(sizeof(struct property_time));
	if (property_time_ptr == NULL) malloc_error();
...
/* ---------------------------------------------------------------------- */
{
	...
	if ((*property_time_ptr1)->time.value*(*property_time_ptr1)->time.input_to_user > (*property_time_ptr2)->time.value*(*property_time_ptr2)->time.input_to_user)
	{
		return 1;
	}
	else if ((*property_time_ptr1)->time.value*(*property_time_ptr1)->time.input_to_user < (*property_time_ptr2)->time.value*(*property_time_ptr2)->time.input_to_user)
	{
		return -1;
	}
	return 0;
...
{
	struct property_time *property_time_ptr;
	if (time_series_realloc(time_series_ptr) == ERROR) return(NULL);
...
/* ---------------------------------------------------------------------- */
{
	if (time_series_ptr == NULL) return(ERROR);
	time_series_ptr->count_properties++;
	time_series_ptr->properties = (struct property_time **) realloc(time_series_ptr->properties, (size_t) time_series_ptr->count_properties * sizeof (struct property_time *));
	if (time_series_ptr->properties == NULL) malloc_error();
...
/* ---------------------------------------------------------------------- */
{
	...
	if (time_ptr1->value*time_ptr1->input_to_user < time_ptr2->value*time_ptr2->input_to_user) {
		return -1;
	} else if (time_ptr1->value*time_ptr1->input_to_user > time_ptr2->value*time_ptr2->input_to_user) {
		return 1;
	}
	return 0;
}
...
/* ---------------------------------------------------------------------- */
{
	if (source == NULL || target == NULL) return(ERROR);
	time_free(target);
	target->type = source->type;
	target->value = source->value;
	target->value_defined = source->value_defined;
	target->input = NULL;
	if (source->input != NULL) target->input = string_duplicate(source->input);
...
/* ---------------------------------------------------------------------- */
{
	...
	qsort (simulation_periods, (size_t) count_simulation_periods, sizeof(double), double_compare);		
	/*
	 *  Keep unique time values
	 */
	k = 0;
	for (i = 1; i < count_simulation_periods; i++) {
		if (simulation_periods[i] >= time_end[count_time_end - 1].value*time_end[count_time_end - 1].input_to_user) {
			warning_msg("Final end time is less than or equal to some time series data.");
			break;
		}
		if (equal(simulation_periods[k], simulation_periods[i], TIME_EPS) == TRUE) {
			continue;
		} else {
			k++;
...
/* ---------------------------------------------------------------------- */
{
	...
	/* *property_ptr = NULL; */
	if (ts_ptr == NULL) return(-1);
	count = ts_ptr->count_properties;
	for (i = 0; i < count; i++) {
		if (equal(cur_time, ts_ptr->properties[i]->time.value*ts_ptr->properties[i]->time.input_to_user, TIME_EPS) == TRUE) {
			*pt_ptr = ts_ptr->properties[i]; 
			return(i);
		}
		if (ts_ptr->properties[i]->time.value*ts_ptr->properties[i]->time.input_to_user > cur_time) break;
	}
	return(-1);
}
/* ---------------------------------------------------------------------- */
 ...
 */	
	...
	while ( (j = copy_token(token, &ptr, &l)) != EMPTY) {
		if (j == DIGIT) {
			...
			input_error++;
			error_msg("Expected list of times with or without trailing time units", CONTINUE);
			return(ERROR);
		}
	}
	return(OK);

 ...
 */
static int
isellipse(minell * me, point * p)
{
	double x = p->x;
	double y = p->y;

	me->mu0 =
		-(me->dy12 * x - me->dx12 * y + me->d12) * (me->dy34 * x -
													me->dx34 * y + me->d34);
	me->lambda0 =
		(me->dy23 * x - me->dx23 * y + me->d23) * (me->dy41 * x -
												   me->dx41 * y + me->d41);

	return (me->alpha * me->lambda0 + me->beta * me->mu0) * me->lambda0 +
		me->gamma * me->mu0 * me->mu0 > 0.0;
}
 ...
 */
static double
minell_calcinsideeval(minell * me)
{
	...
	double DZ =
		2.0 * (u * Du * s + v * Dv * r - Du * v * t - u * Dv * t -
			   u * v * Dt) + u * u * Ds;
...
static int
minell_containspoint(minell * me, point * p)
{
	int i;
...
points_shuffle(int n, point * p)
{
	...
	{
		int nn =
			(int) ((double) n * (double) rand() / ((double) RAND_MAX + 1.0));
		int tmp = numbers[i];
 ...
 */
static int
onleftside(point * p, point * p0, point * p1)
{
	double tmp =
 ...
 */
static void
points_makeccw(int n, point * points[])
{
	point *p;
	int i;
...
static void
minell_calcb(minell * me, int nb, point * pointsb[])
{
	int i;
 ...
 */
static void
points_movelasttofront(point ** points, int n)
{
	point *p = points[n - 1];
...
{
	return _hypot(p1->x - p2->x, p1->y - p2->y);
}
 ...
 */
static void
minell_findfifthpoint(minell * me)
{
	point **ps = me->points;
	int factor = 5;
	int offset = 0;
...
minell_calcprm(minell * me)
{
	...
	else if (me->n == 2)
	{
		...
		me->a = _hypot(me->c.x - ps[0]->x, me->c.y - ps[0]->y);
		me->b = 0.0;
 ...
 */
static void
minell_calc_welzl(minell * me, int n, point * points[], int nb,
				  point * pointsb[])
{
	point *p;
	int count;
 ...
 */
static void
minell_calc(minell * me, int n, point * points[])
{
	int nin;
	int i;
 ...
 */
void
minell_scalepoints(minell * me, int n, point points[])
{
 ...
 */
void
minell_rescalepoints(minell * me, int n, point points[])
{
	double _cos = cos(me->theta);
	double _sin = sin(me->theta);
	double a = me->a;
...
usage()
{
	...
	printf
		("  <file> -- data file with X and Y coords in the first two columns\n");
	...
	printf
		("  -t -- produce stats by running 1000 times with seed changing from\n");
	printf("        1 (unless explicitely specified) to 1000\n");
	printf("  -v -- verbose\n");
	printf("  -V -- very verbose\n");
	printf("  -VV -- even more verbose\n");
	printf
		("Description: `minell' calculates and prints parameters of the ellipse of\n");
...
static void
parse_commandline(int argc, char *argv[], char **fname)
{
	int i;
...
main(int argc, char *argv[])
{
	...
	if (!test)
	{
		...
		{
			long dt =
				1000000 * (tv1.tv_sec - tv0.tv_sec) + tv1.tv_usec -
				tv0.tv_usec;
	...
	else
	{
		...
		fprintf(stdout,
				"  average number of calls to Welzl's procedure = %d\n",
				minell_calc_count / NTEST);
		fprintf(stdout,
				"  average number of ellipses built by 3 support points = %d\n",
				minell_calc3_count / NTEST);
		fprintf(stdout,
				"  average number of ellipses built by 4 support points = %d\n",
				minell_calc4_count / NTEST);
		fprintf(stdout,
				"  average number of in-ellipse test by 3 support points = %d\n",
				minell_eval3_count / NTEST);
		fprintf(stdout,
				"  average number of in-ellipse test by 4 support points = %d\n",
				minell_eval4_count / NTEST);
		fprintf(stdout,
				"  average number of in-ellipse test by 5 support points = %d\n",

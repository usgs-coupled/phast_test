...
void Cube::Points_in_polyhedron(std::list<int> & list_of_numbers, std::vector<Point> &node_xyz)
{
...
void Cube::printOn(std::ostream& os) const
{
	os << "\t" << "-zone"
		<< " " << this->box.x1
		<< " " << this->box.y1
...
bool Cube::Segment_in_cube(Point &p1, Point &p2, Point &i1, Point &i2, double &length, Cube::CUBE_INTERSECTION &c_intersection )
{
  ...
  // X minus 
  pi[0] = Segment_intersect_plane(1.0, 0.0, 0.0, -zo->x1,
    p1, diff, t[0]);
  // Y minus 
  pi[1] = Segment_intersect_plane(0.0, 1.0, 0.0, -zo->y1,
    p1, diff, t[1]);
  // Z minus 
  pi[2] = Segment_intersect_plane(0.0, 0.0, 1.0, -zo->z1,
    p1, diff, t[2]);
  // X plus 
  pi[3] = Segment_intersect_plane(1.0, 0.0, 0.0, -zo->x2,
    p1, diff, t[3]);
  // Y plus 
  pi[4] = Segment_intersect_plane(0.0, 1.0, 0.0, -zo->y2,
    p1, diff, t[4]);
  // Z plus 
  pi[5] = Segment_intersect_plane(0.0, 0.0, 1.0, -zo->z2,
    p1, diff, t[5]);
  ...
  for (i = 0; i < 6; i++)
  {
    ...
    case Cube::PI_SEGMENT:
      if ( 
	(
	(p1.get_coord()[i%3] <= this->p[1].get_coord()[i%3]) && 
  ...
  } else 
  {
    ...
    for (i = 0; i < 6; i++)
    {
      if (pi[i] == Cube::PI_SEGMENT) count_faces++;
    }
    switch (count_faces)
    {
    case 1:

...
void
PHAST_Transform::Transform(Point & p)
{
...
void
PHAST_Transform::Inverse_transform(Point & p)
{
	boost::numeric::ublas::vector < double >v(4);
	v(0) = p.x();
	v(1) = p.y();
...
void
PHAST_Transform::Transform(std::vector < Point > &pts)
{
...
void
PHAST_Transform::Inverse_transform(std::vector < Point > &pts)
{
	std::vector < Point >::iterator it;
	for (it = pts.begin(); it != pts.end(); it++)
	{
